...
!
     ...
     &                PHIP,PHIW,ANFEM,TMELT,TCR,BCH,ICETYPE,
...
!           COMPUTATION OF HEAT FLUXES BETWEEN ATMO AND ICE COVER
     ...
     &          RAINFALL%R(I),SNOW%R(I),INC_SNOW,SUMPH_ICE%R(I),
     &          PHCL%R(I),PHRI%R(I),PHPS%R(I),PHIB%R(I),PHIE%R(I),
     &          PHIH%R(I),PHIP%R(I),ANFEM%R(I),DT,AT,MARDAT,MARTIM,
     &          LAMBD0,1)
            ELSE
              SUMPH_ICE%R(I) = 0.D0
            ENDIF
...
!           RADIATIVE FLUX REMOVED FROM THE BALANCE IF COUPLING WITH 3D
            IF(PRESENT(NPOIN3)) THEN
              SUMPH_ICE%R(I) = SUMPH_ICE%R(I) - PHPS%R(I)
            ENDIF
!
!           COMPUTATION OF HEAT FLUXES BETWEEN ATMO AND OPEN WATER
            CALL THERMAL_FLUXES(TAIR%R(I),TN%ADR(IND_T)%P%R(I+K),
     &        TMELT%R(I),TDEW%R(I),CLDC%R(I),VISBI%R(I),WINDS%R(I),
     &        RAINFALL%R(I),SNOW%R(I),INC_SNOW,SUMPH%R(I),PHCL%R(I),
     &        PHRI%R(I),PHPS%R(I),PHIB%R(I),PHIE%R(I),PHIH%R(I),
     &        PHIP%R(I),ANFEM%R(I),DT,AT,MARDAT,MARTIM,LAMBD0,0)
!
!           RADIATIVE FLUX REMOVED FROM THE BALANCE IF COUPLING WITH 3D
            IF(PRESENT(NPOIN3)) THEN
...
!           ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
     ...
     &        HN(I),VMAG,TN%ADR(IND_T)%P%R(I+K),TMELT%R(I),ICETYPE%I(I))
            PHIW%R(I) = 0.D0
!
!           ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
!           ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
     ...
     &           + (PHIW%R(I)+SUMPH_ICE%R(I))*ANFEM%R(I) )/HN(I)
            TEXP(I) = TEXP(I) + SRCT
!
          ELSE
!
...
!       TODO: CAN BE REPLACED BY VOLU2DPAR
        CALL VECTOR( T1,'=','MASBAS          ',SUMPH%ELM,1.D0,

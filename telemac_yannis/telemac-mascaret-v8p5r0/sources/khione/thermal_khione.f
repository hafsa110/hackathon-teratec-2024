...
!                   *************************
!
     &(TAIR,TWAT,TFRZ,TDEW,CC,VISB,WIND,PLUIE,SNOW,INC_SNOW,SUMPH,PHCL,
     & PHRI,PHPS,PHIB,PHIE,PHIH,PHIP,ANFEM,DT,AT,MARDAT,MARTIM,LAMBD0,
     & CICE)
!
!***********************************************************************
...
!
      ...
      DOUBLE PRECISION, INTENT(IN)    :: DT,AT,SNOW
...
!-----------------------------------------------------------------------
!
!     TODO: U SHOULD BE REPLACED BY |U_WATER - U_ICE|
!     REYNOLDS NUMBER
      DH = 4.D0*H
      RE = U * DH / XNU
!
!     DRY BANKS
      IF(DH.LE.EPS.AND.DH.GT.0.D0) THEN
        WATERICE_HEAT_COEF = 0.D0 !1394.D0
        RETURN
      ENDIF
!
!     LAMINAR FLOW
!     TODO: REMOVE ICETYPE CONDITION WHEN U IS
!           REPLACED BY |U_WATER - U_ICE|
      IF((RE.LT.2200.D0).OR.(ICETYPE.LE.1)) THEN
        WATERICE_HEAT_COEF = ATA*TC_WT/DH
!
      ELSE
!     TURBULENT FLOW
        IF( TWAT.GT.TFRZ ) THEN ! WATER TEMP.>TFRZ

...
!
     ...
     &    SUM_SRC,EPS,ALPHA,CONSTSS,LIMFLAG,IMP)
!
!***********************************************************************
! KHIONE   V8P2
!***********************************************************************
...
!
      ...
      DOUBLE PRECISION, INTENT(IN)     :: EPS,ALPHA
...
!
      INTEGER K,J,KP1,KM1
      INTEGER, PARAMETER :: LKOPT = 0
      DOUBLE PRECISION, PARAMETER :: EPS0 = 1.D-8
      DOUBLE PRECISION TW,QK,LK
...
! ~~>   COMPUTE NUSSLET NUMBER OF FRAZIL CLASS K
        ...
        ELSE IF (NUSSI.EQ.2) THEN
          NUSST(K) = NUSSELT(RK_FRZL(K), EPS, ALPHA)
        ENDIF
!
...
! ~~>     COMPUTING SOURCE TERM OF FRAZIL CONC. OF CLASS K
          ELSE
            KP1 = MIN(K+1, NC_FRA)
            KM1 = MAX(K-1, 1)
            RP1 = (EK_FRZL(KP1)*RK_FRZL(KP1)**2)/
     &            (EK_FRZL(K)*RK_FRZL(K)**2)
            RM1 = (EK_FRZL(KM1)*RK_FRZL(KM1)**2)/
     &            (EK_FRZL(K)*RK_FRZL(K)**2)
!
!           GROWTH
            IF(TW.LT.TFRZ)THEN
...
!
     ...
     &  RK_FRZL,EK_FRZL,VK_FRZL,VBK,SNNMAX
      IMPLICIT NONE
!
!+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
!
...
!           NUCLEATION SINK TERM
            SRCSN(K) = -PI*NTILDE*WRK*CF*RK_FRZL(K)**2
            SRCSN(K) = SRCSN(K)*(EK_FRZL(1)*RK_FRZL(1)**2)/
     &                          (EK_FRZL(K)*RK_FRZL(K)**2)
!           LIMITOR
            MINSRC = -SUM_SRC(K)-CF/DT
            IF (SRCSN(K).LE.MINSRC) THEN
...
!
     ...
     &                                DEPK,EROK,PRE_MIN,MINCK,SURF_EF
      IMPLICIT NONE
!
!+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
!
...
!         (DUE TO PREVIOUS SOURCES)
          IF(LIMFLAG.EQ.0.AND..NOT.MINFLAG) THEN
...
!             *******************************
!
     &  ( I, VMAG, H, KT, EPS, ALPHA, CF, AK, EP, ITURB_TEL)
!
!***********************************************************************
! KHIONE   V8P2
!***********************************************************************
...
!
      ...
      DOUBLE PRECISION, INTENT(INOUT)  :: KT,EPS,ALPHA
!
!+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
!
...
!+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
!
      DOUBLE PRECISION, INTENT(IN)     :: RADIUS, ALPHA, EPS
!
!+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
!
      DOUBLE PRECISION MSTAR, KOLML, PR, PR12, PR13
      DOUBLE PRECISION, PARAMETER :: NUZDEG = 1.792E-6
!
!+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
!
      KOLML = (NUZDEG**3/EPS)**(0.25D0)
      MSTAR = RADIUS/KOLML
      PR = RO0*CP_EAU*NUZDEG/TC_WT
      PR12 = SQRT(PR)
      PR13 = PR**(1.D0/3.D0)
!
...
!       SVENSSON & OMSTEDT (1994) INTERMEDIATE RANGE
        BUOYANCY_VELOCITY = 32.8D0*(2.D0*RADIUS)**1.2D0
...
!
      ELSEIF(IBUOY.EQ.5) THEN
!       MORSE & RICHARD (2009)
        DMM = RADIUS*2.E3
        IF (DMM .LE. 1.27D0) THEN
            BUOYANCY_VELOCITY = 2.025D0*(DMM**1.621D0)
        ELSE
            BUOYANCY_VELOCITY = -0.103D0*(DMM**2.D0)
     &                        + 4.069D0*DMM - 2.024D0
        ENDIF
        BUOYANCY_VELOCITY = BUOYANCY_VELOCITY*1.E-3
!
      ENDIF
!
      RETURN

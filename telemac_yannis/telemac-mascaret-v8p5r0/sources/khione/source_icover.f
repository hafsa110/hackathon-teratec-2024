...
!
     ...
     &                CP_EAU,RO0,RHO_ICE,LH_ICE,
     &                SUM_SRCP,IPREC,SURF_EF
...
!
      ...
      DOUBLE PRECISION            :: SRCTI_TMP,SRCTI_BDI,SRCT
      DOUBLE PRECISION            :: CIDIFF,TIDIFF,CONSTSS
!
      INTRINSIC MAX
!
!=======================================================================
...
!             DYNAMIC ICE LAYER
     ...
     &             (TN%ADR(IND_DCI)%P%R(I).LT.1.D0)) THEN
                  SRCCI_M = -SUM_SRCP%R(I)/((1.D0-SURF_EF)*PRE_MIN)
                ELSE
                  SRCTI_M = -SUM_SRCP%R(I)/(1.D0-SURF_EF)
                ENDIF
!             STATIC ICE LAYER (BORDER ICE)
              ELSE
                IF(ICETYPE%I(I).GT.1) THEN
                  SRCTI_BDI = -SUM_SRCP%R(I)/(1.D0-SURF_EF)
                ENDIF
              ENDIF
            ENDIF
!
...
!           ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            IF(ANFEM%R(I).GT.0.D0) THEN
              CONSTSS = (RHO_ICE*LH_ICE)/(RO0*CP_EAU)
!
              CALL ICOVER_GROWTH( SRCTI_TMP,TN%ADR(IND_T)%P%R(I),
     &          TMELT%R(I),SUMPH_ICE%R(I),THIFEM%R(I),HWI%R(I),DT)
!
!             WATER TEMPERATURE SOURCE DUE TO MELTING/FREEZING
              SRCT = CONSTSS*SRCTI_TMP/HN%R(I)
!
!             DYNAMIC ICE LAYER
              IF((DYN_ICOVER).AND.(ICETYPE%I(I).LE.1)) THEN
...
! ~~>         SOURCE FOR ICE COVER THICKNESS
     ...
     &                                 + SRCTI
            ENDIF
!
! ~~>       EXPLICIT SOURCE TERM FOR WATER TEMPERATURE
!           ******************************************
            IF(ANFEM%R(I).GT.0.D0) THEN

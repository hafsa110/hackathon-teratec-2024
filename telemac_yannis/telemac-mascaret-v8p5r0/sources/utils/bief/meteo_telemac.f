...
!
     ...
     &   CST_RAY3,CST_PVAP,SNOW,INC_SNOW
!
!=======================================================================
!
...
!
!     CLDC: CLOUD COVER
!     > Measured in tenths for KHIONE or octas for WAQTEL
!
...
!
      CALL GET_MESH_DATE(NFMT,NFIC,METEO_REF_DATE,IERR)
     ...
     &                              METEO_REF_DATE(4:6))
        ...
        WRITE(LU,667) METEO_REF_DATE
667     FORMAT(5X,1I4,'-',1I0.2,'-',1I0.2,' ',
     &         1I0.2,':',1I0.2,':',1I0.2)
      ELSE
        METEO_OFFSET = 0.D0
        TEL_OFFSET2  = 0.D0
...
!     READ ONE TIME FRAME AT LEAST
      ...
      DO I = 1,NVALUEB
        CALL GET_DATA_VALUE
     &    (NFMT,NFIC,ITIMEB1-1,CHOIX(I),VALUEB0,NPOINB,IERR)
        CALL CHECK_CALL
...
!     READ A SECOND TIME FRAME IF THERE
      IF( NTIMEB.GT.1 )THEN
        ...
        DO I = 1,NVALUEB
          CALL GET_DATA_VALUE
     &      (NFMT,NFIC,ITIMEB2-1,CHOIX(I),VALUEB0,NPOINB,IERR)
          CALL CHECK_CALL
     &      (IERR,'METEO,INIT_FIC_BINARY:GET_DATA_VALUE:CHOIX')
          DO J = 1,NPOINB
            VALUEB2(I,J) = VALUEB0(J)
...
!     __________________________________________________________________
!     FIND WHEN
      IF( WHEN+TEL_OFFSET2.GE.TIMEB(ITIMEB2) )THEN
 132    ITIMEB1 = ITIMEB2
        DO I = 1,NVALUEB
          DO J = 1,NPOINB
        ...
        ENDDO
        ITIMEB2 = ITIMEB2 + 1
        IF(ITIMEB1.EQ.NTIMEB.AND.
     &     (WHEN+TEL_OFFSET2-TIMEB(ITIMEB1))/MAX(TIMEB(ITIMEB1),1.D-7)
     &        .LT.1.D-7) THEN
          ITIMEB1 = NTIMEB-1
          ITIMEB2 = NTIMEB
        ENDIF
     ...
     &      (IERR,'METEO,INIT_FIC_BINARY:GET_DATA_VALUE:CHOIX')
          ...
          ENDDO
        ENDDO
        IF(ITIMEB2.EQ.NTIMEB.AND.
     &     (WHEN+TEL_OFFSET2-TIMEB(ITIMEB2))/MAX(TIMEB(ITIMEB2),1.D-7)
     &        .LT.1.D-7) THEN
          GOTO 133
        ENDIF
        IF( WHEN+TEL_OFFSET2.GE.TIMEB(ITIMEB2) ) GOTO 132
!
 133    CONTINUE
!
      ENDIF

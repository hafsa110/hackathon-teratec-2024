...
!
      ...
      DO N = 1, NBREACH
        ...
        IF(OPTNBR.EQ.3.OR.OPTNBR.EQ.5) THEN
          READ(IFIC,*,ERR=993) IDUM
          CALL SKIP_COMMENT_LINE(IFIC, COMMENT, IERR)
        ENDIF
!
        IF(OPTNBR.EQ.4) THEN
          READ(IFIC,*,ERR=893) IDUM,IDUM,IDUM
          CALL SKIP_COMMENT_LINE(IFIC, COMMENT, IERR)
          IF(IERR.NE.0) GOTO 900
        ENDIF
!
        IF(OPTNBR.EQ.4.OR.OPTNBR.EQ.5) THEN
          READ(IFIC,*,ERR=894) DDUM
          CALL SKIP_COMMENT_LINE(IFIC, COMMENT, IERR)
          IF(IERR.NE.0) GOTO 900
        ENDIF
...
893   CONTINUE
      ...
      WRITE(LU,*) '               BREACHES DATA FILE'
      WRITE(LU,*) '               FOR THE BREACH ',N
      WRITE(LU,*) '               THE NUMBER OF MESH POINTS'
...
894   CONTINUE
      ...
      WRITE(LU,*) '               BREACHES DATA FILE'
      WRITE(LU,*) '               FOR THE BREACH ',N
      WRITE(LU,*) '               THE THRESHOLD VALUE FOR HYDRAULIC'
      WRITE(LU,*) '               HEAD CANNOT BE READ'
      GO TO 2000
!
999   CONTINUE

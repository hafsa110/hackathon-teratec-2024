...
!
      DO I=1,MAXLU_T2D
        ...
        ELSEIF(T2D_FILES(I)%TELNAME.EQ.'T2DRST') THEN
          T2DRST = I
!       TIME SERIES FILES
        ELSEIF(T2D_FILES(I)%TELNAME.EQ.'T2DHI1') THEN
          T2DHI1=I
        ELSEIF(T2D_FILES(I)%TELNAME.EQ.'T2DHI2') THEN
          T2DHI2=I
        ELSEIF(T2D_FILES(I)%TELNAME.EQ.'T2DCO1') THEN
          T2DCO1=I
        ELSEIF(T2D_FILES(I)%TELNAME.EQ.'T2DCO2') THEN
          T2DCO2=I
!
        ENDIF
      ENDDO
...
!
      ...
      ALLOCATE(OPT_SOFT(MAXFRO))
      ALLOCATE(COEF_SOFT(MAXFRO))
...
!
      ...
      IF(NSOFT.NE.0) THEN
        DO K=1,NSOFT
          OPT_SOFT(K) = MOTINT( ADRESS(1,121) + K-1 )
          IF(OPT_SOFT(K).NE.0) SOFTFR=.TRUE.
        END DO
      ENDIF
...
!
      ...
      IF(NSOFT.NE.0) THEN
        DO K=1,NSOFT
          COEF_SOFT(K) = MOTREA( ADRESS(2,97) + K-1 )
        ENDDO
      ENDIF
      NDEBIT = DIMEN(2,30)
      IF(NDEBIT.NE.0) THEN
...
!     ATMOSPHERIC PRESSURE AT TIDAL BOUNDARY CONDITIONS
      PRESSBC = MOTLOG(ADRESS(3,68))
!
...
!
!     TIME SERIES COORDINATES FILES
      T2D_FILES(T2DCO1)%NAME=MOTCAR( ADRESS(4,104) )
      T2D_FILES(T2DCO2)%NAME=MOTCAR( ADRESS(4,105) )
!     TIME SERIES FILES
      T2D_FILES(T2DHI1)%NAME=MOTCAR( ADRESS(4,106) )
      T2D_FILES(T2DHI2)%NAME=MOTCAR( ADRESS(4,108) )
 !    FORMAT OF THE TIME SERIES FILE
      T2D_FILES(T2DHI1)%FMT = MOTCAR( ADRESS(4,107) )(1:8)
...
!
      ...
      IF(ITURB.NE.3) THEN
        ...
        SORIMP(11) = .FALSE.
      ENDIF
      IF(.NOT.(INCLUS(COUPLING,'WAQTEL').AND.

...
!
      ...
      DOUBLE PRECISION VEL,HEADRIV,HEADFLP,DELTAHEAD
!
...
!
      DO I = 1, NBRECH
        IF((OPTNBR%I(I).EQ.5).OR.(OPTERO%I(I).EQ.9)) THEN
          ALLOCATE(HAA(NBLS(I)))
          ALLOCATE(HVV(NBLS(I)))
!         DIKE3 CONTAINS THE SAME LIST OF POINTS OF DIKE2 BUT IN
!         OPPOSITE ORDER
!         FILLED ONLY IF APPROPRIATE OPTION OTHERWISE VOID
          DO J = 1,NBLS(I)
            DIKE3(I,J) = DIKE2(I,NBLS(I)-J+1)
          ENDDO
        ENDIF
        IF(OPTERO%I(I).EQ.1) THEN
!         CASES WHERE TDECBR NEEDS TO BE DEFINED SINCE NOT GIVEN BY THE USER
...
!           CASE WHERE ONE OF THE ENDS IS NOT IN THE SUB-DOMAIN
            ...
            ENDIF
          ENDIF
          IF(OPTNBR%I(I).EQ.4.AND.TDECBR%R(I).LT.0.D0) THEN
            IF(NUMPSD%I(I).GT.0) THEN
              ZW = ZF%R(NUMPSD%I(I)) + H%R(NUMPSD%I(I))
            ELSE
              ZW = 0.D0
...
!           CASE WHERE ONE OF THE ENDS IS NOT IN THE SUB-DOMAIN
            ...
            ENDIF
!           ENERGY COMPUTATION
            IF(NUMRIV%I(I).GT.0) THEN
              VEL=U%R(NUMRIV%I(I))**2+V%R(NUMRIV%I(I))**2
              HEADRIV = ZF%R(NUMRIV%I(I)) + H%R(NUMRIV%I(I)) +
     &        VEL/DEUXG
            ELSE
              HEADRIV = 0.D0
            ENDIF
            IF(NUMFLP%I(I).GT.0) THEN
              VEL=U%R(NUMFLP%I(I))**2+V%R(NUMFLP%I(I))**2
              HEADFLP = ZF%R(NUMFLP%I(I)) + H%R(NUMFLP%I(I)) +
     &        VEL/DEUXG
            ELSE
              HEADFLP = 0.D0
            ENDIF
!           CASE WHERE WE ARE BETWEEN SEVERAL SUB-DOMAINS
            IF(NCSIZE.GT.1) THEN
              HEADRIV = P_MAX(HEADRIV) + P_MIN(HEADRIV)
              HEADFLP = P_MAX(HEADFLP) + P_MIN(HEADFLP)
            ENDIF
            DELTAHEAD=ABS(HEADRIV-HEADFLP)
!           CRITERIA: THRESHOLD ON FREE SURFACE OVER THE DIKE
!           PLUS DIFFERENCE OF ENERGY BETWEEN FLOW SIDE AND FLOODPLAIN SIDE
!           COMPUTED AT NODES GIVEN BY USER
            IF(ZW.GT.ZDECBR%R(I).AND.DELTAHEAD.GT.THRHEAD%R(I)) THEN
              WRITE(LU,20) I, AT
              TDECBR%R(I) = AT
            ENDIF
          ENDIF
!
          IF(OPTNBR%I(I).EQ.5.AND.TDECBR%R(I).LT.0.D0) THEN
            IF(NUMPSD%I(I).GT.0) THEN
              ZW = ZF%R(NUMPSD%I(I)) + H%R(NUMPSD%I(I))
            ELSE
              ZW = 0.D0
...
!             MAXIMUM VALUE OF HYDRAULIC HEAD
              IF(ABS(HAA(B)-HVV(B)).GT.MAXDH) THEN
                MAXDH = ABS(HAA(B)-HVV(B))
              ENDIF
            ENDDO
!           AVERAGE VALUE OF HYDRAULIC HEAD
            IF(NBLS(I).GT.0) THEN
              DHH = ABS(SUM(HAA)-SUM(HVV))/NBLS(I)
            ENDIF
!           CRITERIA: THRESHOLD ON FREE SURFACE OVER THE DIKE
!           PLUS DIFFERENCE OF ENERGY BETWEEN FLOW SIDE AND FLOODPLAIN SIDE
!           COMPUTED THROUGH A MAX OR AN AVERAGE OF VALUES AT THE FEETS DIKE
!           FOR TEST USERS: REPLACE DHH OR MAXDH
            IF(ZW.GT.ZDECBR%R(I).AND.DHH.GT.THRHEAD%R(I)) THEN
              WRITE(LU,20) I, AT
              TDECBR%R(I) = AT
            ENDIF
          ENDIF
...
!           ENERGY COMPUTATION
            IF(NUMRIV%I(I).GT.0) THEN
              VEL=SQRT(U%R(NUMRIV%I(I))**2+V%R(NUMRIV%I(I))**2)
              HEADRIV = ZF%R(NUMRIV%I(I)) + H%R(NUMRIV%I(I)) +
     &        VEL/DEUXG
            ELSE
              HEADRIV = 0.D0
            ENDIF
            IF(NUMFLP%I(I).GT.0) THEN
              VEL=SQRT(U%R(NUMFLP%I(I))**2+V%R(NUMFLP%I(I))**2)
              HEADFLP = ZF%R(NUMFLP%I(I)) + H%R(NUMFLP%I(I)) +
     &        VEL/DEUXG
            ELSE
              HEADFLP = 0.D0
...
!               TO DECIDE WHEN FORMULA WILL BE USED FOR STUDIES:
!               USING DHH OR MAXDH?
!                DHH = 0.D0
                MAXDH = 0.D0
!

...
!
     ...
     &   (2*INT(WAQPROCESS/2).EQ.WAQPROCESS.OR.
     &    5*INT(WAQPROCESS/5).EQ.WAQPROCESS)) THEN
        CALL BIEF_ALLVEC(1,CSO2,'CSO2  ',IELMH,1,1,MESH)
      ELSE
        CALL BIEF_ALLVEC(1,CSO2,'CSO2  ',    0,1,0,MESH)
      ENDIF
!
...
!     RUNOFF-RAINFALL
      ...
      ENDIF
!
!     VARIABLES TRANSMITTED FROM TOMAWAC TO GAIA
!
      IF(INCLUS(COUPLING,'TOMAWAC')) THEN
        ...
        !surface rollers
        CALL BIEF_ALLVEC(1,SURFROL,'SURROL',IELMH,1,1,MESH)
        CALL BIEF_ALLVEC(1,SURFDIS,'SURDIS',IELMH,1,1,MESH)
      ELSE
        ...
        !surface rollers
        CALL BIEF_ALLVEC(1,SURFROL,'SURROL',0    ,1,0,MESH)
        CALL BIEF_ALLVEC(1,SURFDIS,'SURDIS',0    ,1,0,MESH)
! Cv added
        FRICOU = .FALSE.
      ENDIF
!
...
!                                                       WAC2
      ...
      IF(INCLUS(COUPLING,'TOMAWAC2')) THEN
        ...
        IF (INCLUS(COUPLING,'GAIA')) THEN
          ...
          NVARTOM2TEL = NVARTOM2TEL + 7
          CALL ADDBLO(TOM2TEL,SURFROL)
          CALL ADDBLO(TOM2TEL,SURFDIS)
        ENDIF
        IF(COUROU) THEN
...
! 41  OXYGEN SATURATION CONCENTRATION
      CALL ADDBLO(VARSOR,CSO2)
! 42  PERCENTAGE OF OXYGEN SATURATION
      CALL ADDBLO(VARSOR,T12)
!

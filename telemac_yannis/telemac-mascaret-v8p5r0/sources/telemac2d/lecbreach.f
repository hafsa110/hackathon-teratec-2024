...
!
      IF(NBRECH.GT.0) THEN
        ...
        CALL BIEF_ALLVEC(1,THRHEAD,'TRHHEA',NBRECH,1,0,MESH)
        ...
        CALL BIEF_ALLVEC(2,NUMRIV,'NUMRIV',NBRECH,1,0,MESH)
        CALL BIEF_ALLVEC(2,NUMFLP,'NUMFLP',NBRECH,1,0,MESH)
      ...
      ELSE
        ...
        CALL BIEF_ALLVEC(1,THRHEAD,'THRHEA',0,1,0,MESH)
        ...
        CALL BIEF_ALLVEC(2,NUMRIV,'NUMRIV',0,1,0,MESH)
        CALL BIEF_ALLVEC(2,NUMFLP,'NUMFLP',0,1,0,MESH)
...
!
      DO N = 1, NBRECH
        ...
        CALL SKIP_COMMENT_LINE(IFIC, COMMENT, IERR)
        IF(IERR.NE.0) GOTO 900
        IF(OPTNBR%I(N).EQ.3.OR.OPTNBR%I(N).EQ.5) THEN
          ...
          ENDIF
        ENDIF
        IF(OPTNBR%I(N).EQ.4) THEN
          READ(IFIC,*,ERR=893) NUMPSD%I(N),NUMRIV%I(N),NUMFLP%I(N)
          CALL SKIP_COMMENT_LINE(IFIC, COMMENT, IERR)
          IF(IERR.NE.0) GOTO 900
          IF(NCSIZE.GT.1) THEN
            NUM = NUMPSD%I(N)
          ...
          ENDIF
        ENDIF
        IF(OPTNBR%I(N).EQ.4.OR.OPTNBR%I(N).EQ.5) THEN
          READ(IFIC,*,ERR=894) THRHEAD%R(N)
          CALL SKIP_COMMENT_LINE(IFIC, COMMENT, IERR)
          IF(IERR.NE.0) GOTO 900
        ENDIF
...
!       MESH POINTS IN THE BREACH DOMAIN
        ...
        ENDDO
!
!       DIKE1 AND DIKE2 ARE FILLED ONLY IF APPROPRIATE OPTION
        IF((OPTNBR%I(N).EQ.5).OR.(OPTERO%I(N).EQ.9)) THEN
...
!
893   CONTINUE
      WRITE(LU,*) 'LECBRECH : READ ERROR ON THE'
      WRITE(LU,*) '           BREACHES DATA FILE'
      WRITE(LU,*) '           FOR THE BREACH ',N
      WRITE(LU,*) '           THE NUMBER OF MESH POINTS CANNOT BE READ'
      GO TO 2000
!
894   CONTINUE
      WRITE(LU,*) 'LECBREACH : READ ERROR ON THE'
      WRITE(LU,*) '            BREACHES DATA FILE'
      WRITE(LU,*) '            FOR THE BREACH ',N
      WRITE(LU,*) '            THE THRESHOLD VALUE FOR HYDRAULIC HEAD'

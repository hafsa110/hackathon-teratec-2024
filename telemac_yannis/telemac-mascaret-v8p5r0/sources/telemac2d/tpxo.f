...
!+        17/05/2023
!+        V8P5
!+        Compatibility with HAMTIDE from Hamburg University
!
!history  C. COULET (BRGM)
!+        13/06/2023
!+        V8P5
!+        Pressure effect at tidal boundaries (+ initial conditions)
!
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      ...
      DO IC = 1,NC0
        ...
        DO JC = 1,TPXO_NCMX
          IF( CID(IC).EQ.TPXO_CONSTID(JC) ) THEN
            IND(K) = JC
!           WRITE(LU,*) JC,IC,CID(IC),TPXO_CONSTID(JC),NC0 ! DKAF
            EXIT
          ENDIF
        ENDDO
        IF( IND(K).EQ.0 ) THEN
...
!
     ...
     & I_ORIG,J_ORIG,HMIN_VIT_IC,VITINI_TPXO,VIT_IN_T2DBB2,
     & PRESSBC,PATMOS,GRAV,ROEAU)
!
!***********************************************************************
! TELEMAC2D   V8P5
!***********************************************************************
...
!|  PRESSBC       |-->| LOGICAL TO ADD ATMOSPHERIC PRESSURE ON BOUNDARY OR NOT
!|  ROEAU         |-->| WATER DENSITY
...
!|  VIT_IN_T2DBB2 |-->| IF YES, VELOCITIES ARE GIVEN IN TIDAL DATA BASE 2
!|                |   | NOT CIRCULATIONS
...
!
      ...
      DOUBLE PRECISION, INTENT(IN)    :: GRAV,ROEAU
      DOUBLE PRECISION, INTENT(IN)    :: PATMOS(NPOIN)
      ...
      LOGICAL, INTENT(IN)             :: PRESSBC
      LOGICAL, INTENT(INOUT)          :: INTMICON,VITINI_TPXO
      LOGICAL, INTENT(IN)             :: VIT_IN_T2DBB2
!
!+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
!
...
!       FOLLOWING LINE NOT NEEDED FOR HAMTIDE IF BINARY FILE CORRECTLY
!       GENERATED:
!       ZT_I(IC,1:N,1:M) = -ZT_I(IC,1:N,1:M)
...
!               ALSO TAKEN INTO ACCOUNT
          IF(PRESSBC) THEN
            HREF0 = MSL - (PATMOS(IPOIN) - 101325.D0) / (ROEAU * GRAV)
          ELSE
            HREF0 = MSL
          ENDIF
!          IF(H(IPOIN).LE.-MSL) THEN
          IF(H(IPOIN).LE.-HREF0) THEN
            H(IPOIN) = 0.D0
          ELSE
!            H(IPOIN) = H(IPOIN) + MSL
            H(IPOIN) = H(IPOIN) + HREF0
          ENDIF
!
        ENDDO
...
!
     ...
     & I_ORIG,J_ORIG,INTMICON,HMIN_VIT_BC,VIT_IN_T2DBB2,
     & PRESSBC,PATMOS,GRAV,ROEAU)
!
!***********************************************************************
! TELEMAC2D   V8P5
...
!
     ...
     &        + MSL
!           PRESSURE EFFECT IS DONE OUTSIDE IN TIDAL_MODEL_T2D/T3D
            HBTIDE%R(K) = Z-ZF(NBOR(K))
!           HBTIDE%R(K) = MAX( 0.D0 , Z-ZF(NBOR(K)) )
!           DONE IN TIDAL_MODEL_T2D/T3D FROM V8P5
          ELSEIF(TIDALTYPE.GE.2.AND.TIDALTYPE.LE.6) THEN
...
!  IF LIHBOR(K).EQ.4, Z IS NOT CALCULATED BEFORE
     ...
     &        + MSL
            IF(PRESSBC) THEN
              Z = Z - (PATMOS(NBOR(K)) - 101325.D0) / (ROEAU * GRAV)
            ENDIF

...
!
      ...
      USE OUT_HISTORY, ONLY : HISTPERIOD,YAHIST
!
...
!     CHECK IF TIME SERIES OUTPUT IS NEEDED
      IF(YAHIST) THEN
        CALL HISTPERIOD('T2D   ',AT,LEOHIS)
      ELSE
        LEOHIS = .FALSE.
      ENDIF
      LEOCALC = LEO.OR.LEOHIS
!
!-----------------------------------------------------------------------
!
...
!     NO PRINTOUT REQUIRED (LISTING OR RESULT FILE): EXITS
      IF(.NOT.(LEOCALC.OR.IMP)) GO TO 1000
!
!
!=======================================================================
...
!=======================================================================
!
      IF((LEOCALC.AND.SORLEO(3)).OR.(IMP.AND.SORIMP(3))) THEN
        CALL CPSTVC(ZF,FU)
        DO N=1,NPOIN
...
!=======================================================================
!
      IF((LEOCALC.AND.SORLEO(5)).OR.(IMP.AND.SORIMP(5))) THEN
        CALL CPSTVC(ZF,FV)
        DO N=1,NPOIN
          FV%R(N) = H%R(N)+ZF%R(N)
        ENDDO
...
!=======================================================================
!
      IF((LEOCALC.AND.SORLEO(7)).OR.(IMP.AND.SORIMP(7))) THEN
        CALL CPSTVC(ZF,T2)
        DO N=1,NPOIN
          HHH = MAX( H%R(N) , 1.D-8 )
...
!=======================================================================
!
      IF((LEOCALC.AND.SORLEO(8)).OR.(IMP.AND.SORIMP(8))) THEN
        CALL CPSTVC(ZF,T3)
        DO N=1,NPOIN
...
!=======================================================================
!
      IF(((LEOCALC.AND.SORLEO(12)).OR.(IMP.AND.SORIMP(12)))
     &   .AND.ITURB.EQ.2) THEN
        DO N=1,NPOIN
...
!=======================================================================
!
      IF((LEOCALC.AND.SORLEO(13)).OR.(IMP.AND.SORIMP(13))) THEN
        CALL CPSTVC(ZF,T4)
        DO N=1,NPOIN
          T4%R(N)=H%R(N)*U%R(N)
        ENDDO
...
!=======================================================================
!
      IF((LEOCALC.AND.SORLEO(14)).OR.(IMP.AND.SORIMP(14))) THEN
        CALL CPSTVC(ZF,T5)
        DO N=1,NPOIN
          T5%R(N)=H%R(N)*V%R(N)
        ENDDO
...
!=======================================================================
!
      IF((LEOCALC.AND.SORLEO(15)).OR.(IMP.AND.SORIMP(15))) THEN
...
!=======================================================================
!
      IF((LEOCALC.AND.SORLEO(20)).OR.(IMP.AND.SORIMP(20))) THEN
        IF(LT.EQ.0) CALL OS('X=0     ',X=T7)
      ENDIF
      IF((LEOCALC.AND.SORLEO(21)).OR.(IMP.AND.SORIMP(21))) THEN
...
!=======================================================================
!
      IF((LEOCALC.AND.SORLEO(22)).OR.(IMP.AND.SORIMP(22))) THEN
!                             IELM
...
!=======================================================================
!
      IF((LEOCALC.AND.SORLEO(31)).OR.(IMP.AND.SORIMP(31))) THEN
        CALL CPSTVC(CF,T7)
        DO N=1,NPOIN
...
!
      ...
      DO I = 1,NADVAR
        IF((LEOCALC.AND.SORLEO(J)).OR.(IMP.AND.SORIMP(J))) THEN
...
!
!=======================================================================
! COMPUTES PERCENTAGE OF OXYGEN SATURATION
!=======================================================================
!
      IF((LEO.AND.SORLEO(42)).OR.(IMP.AND.SORIMP(42))) THEN
        CALL CPSTVC(ZF,T12)
        DO N=1,NPOIN
          T12%R(N) = T%ADR(IND_O2)%P%R(N)/CSO2%R(N)*100.D0
        ENDDO
      ENDIF
!
!=======================================================================

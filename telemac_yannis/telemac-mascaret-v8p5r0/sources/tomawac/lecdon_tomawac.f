...
!
      DO I=1,MAXLU_WAC
        ...
        ELSEIF(WAC_FILES(I)%TELNAME.EQ.'IMPSPE') THEN
          IMPSPE=I
!       TIME SERIES FILES
        ELSEIF(WAC_FILES(I)%TELNAME.EQ.'WACHI2') THEN
          WACHI2=I
        ELSEIF(WAC_FILES(I)%TELNAME.EQ.'WACCOO') THEN
          WACCOO=I
        ENDIF
      ENDDO
!
...
!     SEE NEXT BAJ FOR CONSEQUENCES ON PARAMETERS
      ...
      SUBSTEP_SURFROL = MOTINT( ADRESS(1, 52) )
!
...
!     VEGETATION
      ...
      BETA_S_SURFROL   = MOTREA( ADRESS(2, 97) )
      BETA_2_SURFROL   = MOTREA( ADRESS(2, 98) )
!
! LOGICAL KEYWORDS
!
      ...
      IF(PART.NE.-1.AND.VENSTA) THEN
        WRITE(LU,*) 'WHEN YOU COUPLE WITH TELEMAC, ', 
     &       'YOU CAN NOT HAVE A STATIONARY WIND IN TOMAWAC, ',
     &       'WIND MUST BE DEFINED IN TELEMAC'
        CALL PLANTE(1)
      ENDIF
      IF (((ABS(VX_CTE).GE.1D-8).OR.(ABS(VY_CTE).GE.1D-8))
     &     .AND..NOT.VENSTA) THEN
        WRITE(LU,*) 'YOU DEFINE A CONSTANTE WIND BUT YOU DO NOT HAVE A '
     &       ,'STATIONARY WIND'
        CALL PLANTE(1)
      ENDIF        
...
!     TIME SERIES COORDINATES FILE
      WAC_FILES(WACCOO)%NAME=MOTCAR( ADRESS(4,28) )
!     TIME SERIES FILE
      WAC_FILES(WACHI2)%NAME=MOTCAR( ADRESS(4,29) )
...
! ONLY CALCULATION OF DISSIPATION WHEN SURFACE ROLLERS OR OUTPUT
      IF (SURF_ROL.OR.SORLEO(38)) THEN
        CALC_DISSIP = .TRUE.
      ELSE
        CALC_DISSIP = .FALSE.
      ENDIF

! NO SURFACE ROLLER OUTPUT WITHOUT SURFACE ROLLERS
      IF (.NOT.SURF_ROL) THEN
        SORLEO(37) = .FALSE.
        SORLEO(39) = .FALSE.
      ENDIF

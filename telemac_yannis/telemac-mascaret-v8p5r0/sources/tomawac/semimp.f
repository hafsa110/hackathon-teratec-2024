...
!
     ...
     &  WAC_CPL_RUN, APISET, DISSIP_BREAK
...
!
          IF((SVENT.GE.2.OR.(LVENT.EQ.1.AND.SVENT.NE.1).OR.
     &        (SMOUT.EQ.2.AND.SVENT.NE.1)).AND..NOT.APISET) THEN
...
!
        ENDIF
!
        IF(VENT.AND.SVENT.EQ.1.AND..NOT.APISET) THEN
...
!
        ENDDO
        DO IP=1,NPOIN2
          DISSIP_BREAK%R(IP) = DISSIP_BREAK%R(IP)/DTSI
        ENDDO
!
        ENDIF
!

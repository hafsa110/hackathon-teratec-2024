...
!
     ...
     &   MESH_TOM=>MESH, WACGEO, COUROU_TEL, VENT_TEL, COPGAI_TEL
...
!
      ...
      CHARACTER(LEN=24), PARAMETER :: CODE4='GAIA                    '
...
!
     ...
     &                   DUMMY,DUMMY)
!
        CALL BIEF_OPEN_FILES(CODE4,GAI_FILES,MAXLU_GAI,PATH,NCAR,
     &                       4,.FALSE.)
!
...
!
      IF(INCLUS(COUPLING,'TOMAWAC')) THEN
        ...
        CALL DEALL_TOMAWAC()
        IF(INCLUS(COUPLING,'TOMAWAC2')
     &       .OR.INCLUS(COUPLING,'TOMAWACT3D2')) CALL END_COUPLE()
      ENDIF
!
      IF(INCLUS(COUPLING,'WAQTEL')) THEN
        CALL CONFIG_CODE(4)

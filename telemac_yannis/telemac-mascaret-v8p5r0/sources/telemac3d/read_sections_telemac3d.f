!                   **********************************
                    SUBROUTINE READ_SECTIONS_TELEMAC3D
!                   **********************************
!
!
!***********************************************************************
...
!+        05/03/2023
!+        V8P5
!+   Creation from TELEMAC-2D version read_sections_telemac2d.f
!
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
!
      USE BIEF, ONLY: NCSIZE
      USE DECLARATIONS_TELEMAC3D, ONLY: MESH2D, CHAIN, NCP, CTRLSC,
     &                                  T3D_FILES, T3DSEC
...
!-----------------------------------------------------------------------
!
!      WRITE(LU,*) '-> ENTERING READ_SECTIONS_TELEMAC3D'
      INP=T3D_FILES(T3DSEC)%LU
      ...
      CASE (:-1) ! SECTION TERMINAL POINTS PROVIDED AS GLOBAL NODES
        DO N=1,NSEC
          ...
          IF (NCSIZE>1) THEN
            ...
            CHAIN(N)%XYEND(:)=0.0D0
          ELSE
            CHAIN(N)%XYBEG(:)= (/MESH2D%X%R(CHAIN(N)%NPAIR(1)),
     &                           MESH2D%Y%R(CHAIN(N)%NPAIR(1))/)
            CHAIN(N)%XYEND(:)= (/MESH2D%X%R(CHAIN(N)%NPAIR(2)),
     &                           MESH2D%Y%R(CHAIN(N)%NPAIR(2))/)
          ENDIF
          CHAIN(N)%NSEG=-1
...
!        END DO
     ...
     &                + (CHAIN(N)%XYEND(2)-YA)**2 )
          CHAIN(N)%NPAIR(1)=1
          CHAIN(N)%NPAIR(2)=1
          DO I=2,MESH2D%NPOIN ! COMPUTATIONALLY INTENSIVE
            XA=MESH2D%X%R(I)
            YA=MESH2D%Y%R(I)
      ...
      CASE (1:) ! PARTITIONED, INSTEAD OF TERMINAL POINTS, PROVIDED AS CHAINS
        DO N=1,NSEC
          ...
          IF (CHAIN(N)%NSEG>0) THEN
            ...
            IF (ERR/=0) THEN
              WRITE(LU,*) 'READ_SECTIONS_TELEMAC3D: ',
...
!      WRITE (LU,*) 'TELEMAC NCP WAS: ',NCP
      ...
      IF (ALLOCATED(CTRLSC)) THEN
        ...
        IF (ERR/=0) THEN
          WRITE(LU,*)
     &    'READ_SECTIONS_TELEMAC3D: ERROR BY DEALLOCATING CTRLSC:',ERR
          CALL PLANTE(1)
          STOP
        ENDIF
      ENDIF
      ...
      IF (ERR/=0) THEN
        WRITE(LU,*)
     &  'READ_SECTIONS_TELEMAC3D: ERROR BY REALLOCATING CTRLSC:',ERR
        CALL PLANTE(1)
        STOP
      ENDIF
      I=1
...
!-----------------------------------------------------------------------
!
!      WRITE(LU,*) '-> LEAVING READ_SECTIONS_TELEMAC3D'
      RETURN
      END SUBROUTINE READ_SECTIONS_TELEMAC3D

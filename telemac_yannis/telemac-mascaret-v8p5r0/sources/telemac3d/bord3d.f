...
!             SIZE OF T3_01 ARRAY = NPOIN3 WITH LOCAL INDEX NOT GLOBAL
     ...
     &                      ' IN THE ASCII FILE'
              ...
              ENDIF
              IF(NCSIZE.GT.1) THEN
                N=MESH3D%KNOLG%I(NBOR2%I(K)+(NP-1)*NPOIN2)
              ELSE
                N=NBOR3%I(IBORD)
              ENDIF
            ...
            ENDIF
!           CASE OF A VERTICAL PROFILE
            IF(VERPROVEL(IVIT).NE.1) THEN
              PROFZ = VEL_PROF_Z(IVIT,NBOR2%I(K),NP,VERPROVEL(IVIT))
              UBORL%R(IBORD) = UBORL%R(IBORD)*PROFZ
              VBORL%R(IBORD) = VBORL%R(IBORD)*PROFZ
            ENDIF
          ENDDO
        ELSE
        ...
        DO K=1,NPTFR2
          IF(LIHBOR%I(K).EQ.KENT) THEN
            ...
            IF(OPT_SOFT(IFRLIQ).NE.0) THEN
              VNORM = U2D%R(NBOR2%I(K)) * XNEBOR2%R(K)
     &              + V2D%R(NBOR2%I(K)) * YNEBOR2%R(K)
              IF(OPT_SOFT(IFRLIQ).EQ.1) THEN
                HBOR%R(K) = HBOR%R(K) + COEF_SOFT(IFRLIQ) * VNORM

...
!
      IF(INCLUS(COUPLING,'TOMAWAC')) THEN
        ...
        !surface rollers
        CALL BIEF_ALLVEC(1,SURFROL,'SURROL',IELMH,1,1,MESH2D)
        CALL BIEF_ALLVEC(1,SURFDIS,'SURDIS',IELMH,1,1,MESH2D)
      ...
      ELSE
        ...
        !surface rollers
        CALL BIEF_ALLVEC(1,SURFROL,'SURROL',0    ,1,0,MESH2D)
        CALL BIEF_ALLVEC(1,SURFDIS,'SURDIS',0    ,1,0,MESH2D)
...
!
     ...
     &    5*INT(WAQPROCESS/5).EQ.WAQPROCESS)) THEN
        CALL BIEF_ALLVEC(1,CSO2,'CSO2  ',IELM3,1,1,MESH3D)
      ELSE
        CALL BIEF_ALLVEC(1,CSO2,'CSO2  ',     0,1,0,MESH3D)
      ENDIF
!
!     IF (ATMOS) THEN
...
!
      ...
      CALL ADDBLO(VARSOR,T2_18)           ! OXYGEN SATURATION CONCENTRATION
      CALL ADDBLO(VARSOR,T2_19)           ! PERCENTAGE OF OXYGEN SATURATION
...
! 32 : CSO2
      CALL ADDBLO(VARSO3,CSO2)
! 33 : O2SAT
      CALL ADDBLO(VARSO3,T3_04)
! 34 : NUTILD (SPALART-ALLMARAS)
...
!-----------------------------------------------------------------------
!
!     AS DONE IN 2D, IF REQUIRED, WE READ HERE THE INPUT SECTIONS FILE
!     AND MODIFY NCP AND CTRLSC(1:NCP) ACCORDINGLY IN READ_SECTIONS
!
!     IN 3D, METHOD WITH SECTIONS DEFINED IN THE STEERING FILE
!     IS NOT IMPLEMENTED
!
      IF(TRIM(T3D_FILES(T3DSEC)%NAME).NE.'') THEN
        CALL READ_SECTIONS_TELEMAC3D
      ENDIF
!
...
!
      ...
      IF(NCP.GT.1.AND.COMFLU) THEN
        CALL BIEF_ALLVEC_IN_BLOCK(MSKSEC,NCP/2,1,'MSKS  ',IELM0,1,1,
     &                            MESH2D)
      ENDIF
!
!-----------------------------------------------------------------------

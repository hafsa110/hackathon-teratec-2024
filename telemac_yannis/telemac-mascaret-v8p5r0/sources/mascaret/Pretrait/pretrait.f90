...
!--TAPENADE
   ...
   integer, dimension(:)         , pointer       :: SectionStoSing => null() 
   ! Lois hydrauliques
...
!--TAPENADE
   ...
   integer           :: isto            ! Compteur sur les stockage
   ...
   integer           :: NbProf        ! Nombre de profil stocker
   integer           :: NbSectEff     ! Nombre de Section stocker
   integer           :: NbSectSing    
...
!
   ...
   if( OptionStockage /= STOCKAGE_TOUTES_SECTION .and. &
      OptionStockage /= STOCKAGE_LISTE_SECTION .and. &
      OptionStockage /= STOCKAGE_SECTION_AV_AM_SING) then   
      Erreur%Numero = 305
      Erreur%ft     = err_305
      Erreur%ft_c   = err_305c
      call TRAITER_ERREUR( Erreur , 'Option de stockage' , '1 ou 2 ou 3' )
      return
   end if

   if( OptionStockage == STOCKAGE_TOUTES_SECTION ) then
      write(UniteListing,10432) 'A toutes les sections'
   elseif( OptionStockage == STOCKAGE_SECTION_AV_AM_SING ) then
      write(UniteListing,10432) 'les sections amont et aval de singularite + profils'
   ...
   if( OptionStockage == STOCKAGE_SECTION_AV_AM_SING ) then
      ! Profil section
      NbProf = size(Profil)
      allocate(SectionStoTmp(NbProf) , STAT = retour)
      if( retour /= 0 ) then
          Erreur%Numero = 5
          Erreur%ft     = err_5
          Erreur%ft_c   = err_5c
          call TRAITER_ERREUR( Erreur , 'SectionStoTmp' )
          return
      end if
      
      do isect = 1 , NbProf
          num_branche = profil(isect)%NumBief
          ...
          absc_abs = profil(isect)%AbsAbs
  
          call XINDIC_S( SectionStoTmp(isect), &
                        absc_abs              , &
                        X                     , &
                        Erreur                )
          if( Erreur%Numero /= 0 ) then
      ...
      ! Singularity section
      NbSectSing = size(Singularite) * 2
      allocate(SectionStoSing(NbSectSing),STAT = retour)
      if( retour /= 0 ) then
        Erreur%Numero = 5
        Erreur%ft     = err_5
        Erreur%ft_c   = err_5c
        call TRAITER_ERREUR( Erreur , 'SectionStoSing' )
        return
      end if
      do ising = 1 , size(Singularite)
        SectionStoSing((ising-1)*2+1) = Singularite(ising)%Section
        SectionStoSing((ising-1)*2+2) = Singularite(ising)%Section +1
      end do 
  
      ! Totality section
      NbSectEff = NbSectSing + NbProf
      ! removing duplicate sections
      do isect = 1, NbProf
        do ising = 1,  size(SectionStoSing)
          if(SectionStoTmp(isect) == SectionStoSing(ising)) then
              NbSectEff = NbSectEff - 1
          end if 
        end do
      end do 

      ! allocation de la table
      allocate(SectionStockage(NbSectEff) , STAT = retour)
      if( retour /= 0 ) then
          Erreur%Numero = 5
          Erreur%ft     = err_5
          Erreur%ft_c   = err_5c
      ...
      !Fill the section in SectionStockage
      isto = 1
      ising = 1
      do isect = 1 , NbSectEff
          if(ising > NbSectSing .and. isto <= NbProf) then
            SectionStockage(isect) = SectionStoTmp(isto)
            isto = isto + 1
          elseif(isto > NbProf .and. ising <= NbSectSing) then
            SectionStockage(isect) = SectionStoSing(ising)
            ising = ising + 1
          else 
            if(SectionStoTmp(isto) < SectionStoSing(ising)) Then
              SectionStockage(isect) = SectionStoTmp(isto)
              isto = isto + 1
            elseif(SectionStoSing(ising) < SectionStoTmp(isto)) Then
              SectionStockage(isect) = SectionStoSing(ising)
              ising = ising + 1
            else !(SectionStoTmp(isto)==SectionStoSing(ising))
              SectionStockage(isect) = SectionStoTmp(isto)
              isto = isto + 1
              ising = ising + 1
            end if
      ...
      deallocate(SectionStoTmp)
      deallocate(SectionStoSing)
      write(*,*) SectionStockage, size(SectionStockage)

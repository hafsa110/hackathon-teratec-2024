...
!--TAPENADE
   ...
   if (OptionCourlis) Then
      ...
      Allocate (Hsed(size(ProfilCourlis)), STAT=retour)  ! Global variable allocation
      Allocate (mySm (size(Profil)), STAT=retour)        ! Global variable allocation

      Hsed(:) = 0.0D0
      mySm(:) = 0.0D0

   endif
      Allocate (myZsl(size(X)), STAT=retour)        ! Global variable allocation
      myZsl(:) = 0.0D0
...
!    End if
 ...
 Else ! these variable must be declared to avoid a segmentation fault when OptionCourlis equals False
...
!  if (TEMPS > Tlim) print *," CouplageHydroSedim" !dbug
  CouplageHydroSedim : Select case (phase_couplage_courlis)
    ...
    Case (PHASE_INITIALISATION)
      ...
      ! TempsCourlis, TfinCourlis, V1, S1, P1, etc. are initialized later in "phase_courlis = INITIALISATION"
      Iter_courlis = 0
    ...
    Case (PHASE_CALCUL)
      ...
      If (phase_hydro == PHASE_CALCUL) Then
         ...
         ! Update of geometry and new "planimetrage" calculation
         phase_planim    = PHASE_CALCUL
      ...
      If (phase_courlis == PHASE_CALCUL) Then
        ...
        !clipping
        !Do i = 1, size(ProfilCourlis)
        Do i = 2, size(ProfilCourlis) - 1
        ! TODO : check if clipping is problematic with Sarap (or for every kernels?)
  ...
  !DeltaH is not identical in suspension transport mode and in bedload transport mode
  ! => there is specific update for each mode
  if (OptionCourlis) then
...
!--TAPENADE
   ...
   CalculSedim : If ((phase_courlis == PHASE_INITIALISATION) .or. &
    ...
    Endif
      ...
      !=========================================================================
      ! INITIALIZATION FOR COURLIS PROFILE EVOLUTION
      !=========================================================================
      IF(BEDLOAD_OPTION) THEN
         CALL PROF_EVOL%INIT(PROFIL, PROFILPLAN, PROFILCOURLIS)
      END IF
...
!if (TEMPS > Tlim) print *," Appel EcrFicConc" !dbug
    ...
    !------ free PROFIL_EVOLUTION structure
    IF(BEDLOAD_OPTION) THEN
      CALL PROF_EVOL%FREE
    ENDIF
    !--------------------------------------

    print *,"" ! line break after displaying current time
    print *," CPT_PLANIM = ",my_cpt_planim

  Endif TermineCourlis ! End of COURLIS result files save

  Deallocate (Vsed, myZsl)  ! Deallocation of sedimentology global variables
  if(OptionCourlis)then

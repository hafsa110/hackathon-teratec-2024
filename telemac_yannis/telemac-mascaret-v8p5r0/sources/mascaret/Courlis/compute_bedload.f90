...
! Module for evolution of the river's profiles
USE M_PROFIL_UTILS, ONLY:&
     PROF_EVOL_UNIF, &  ! flat deposit - uniform erosion
     PROF_EVOL_SIMP     ! uniform deposit and erosion
USE M_PROFIL_EVOLUTION,  &
     PROF_EVOL => PROFIL_EVOLUTION ! rename
...
! AJOUT PU2016
  ...
  real(DOUBLE), dimension(:)  , allocatable :: flux_cell                                ! sum of numerical flux on cells (for VF schemes)
  real(DOUBLE)                              :: flux_qs                                  ! numerical sediment flux (for VF schemes)
  real(DOUBLE)                              :: flux_qsl, flux_qsr                       ! numerical sediment flux at boundaries
...
!=========================================================================
  ...
  Allocate(flux_cell(NbProfil),STAT=retour)
...
10 continue
  ...
  ! Maj des cotes de fond:
  !-----------------------

  ! ******************
  ! Schema VF décentré
  ! ******************
  if (bedload_scheme.eq.1) then
    ! compute numerical flux (loop on interfaces)
    do i = 1, NbProfil-1
        ! upwind scheme
        if (Fr(i+1)<1._DOUBLE) then
          flux_qs = Qsed(i+1)
        else
          flux_qs = Qsed(i+2)
        endif
        ! centered
        !flux_qs = 0.5d0*(Qsed(i+1) + Qsed(i+2))

        ! update flux cells
        flux_cell(i)   = flux_cell(i)   + flux_qs
        flux_cell(i+1) = flux_cell(i+1) - flux_qs
    enddo

    ! compute numerical flux at left boundary
    if (Fr(1)<1._DOUBLE) then
      flux_qsl = Qsed(1)
    else
      flux_qsl = Qsed(2)
    endif
    flux_cell(1) = flux_cell(1) - flux_qsl

    ! compute numerical flux at right boundary
    if (Fr(NbProfil+1)<1._DOUBLE) then
      flux_qsr = Qsed(NbProfil+1)
    else
      flux_qsr = Qsed(NbProfil+2)
    endif
    flux_cell(NbProfil) = flux_cell(NbProfil) + flux_qsr

    ! update (loop on cells)
    do i = 1, NbProfil
        ! collocated control cells
        if (fvcell.eq.1) then
          if (i.eq.1) then
            cellsize = (Yj(2)-Yj(1)) / W2
          else if (i.eq.NbProfil) then
            cellsize = (Yj(NbProfil)-Yj(NbProfil-1)) / W2
          else
            cellsize = (Yj(i+1)-Yj(i-1)) / W2
          endif

        ! stagered control cells
        else if (fvcell.eq.2) then
          cellsize = (Yi(i+2)-Yi(i+1))
        endif

        ! update
        dAz(i) = - DtCourlis * flux_cell(i) / cellsize
    enddo

  ! *************
  ! Schema PU2017
  ! *************
  else if (bedload_scheme.eq.2) then
    ...
    Do i = 1,NbProfil
      ...
      if ( (Fr(i)>=1._DOUBLE).AND.(Fr(i+1)>=1._DOUBLE) ) then
        dAz(i) = - DtCourlis * (Qsed(i+2) - Qsed(i+1)) / cellsize
      else if ( (Fr(i)<1._DOUBLE).AND.(Fr(i+1)<1._DOUBLE) ) then
        dAz(i) = - DtCourlis * (Qsed(i+1) - Qsed(i)) / cellsize
      else if ( (Fr(i)<1._DOUBLE).AND.(Fr(i+1)>=1._DOUBLE) ) then
        dAz(i) = - DtCourlis * 0.5D0 * (Qsed(i+2) - Qsed(i)) / cellsize
      else
        dAz(i) = w0
      endif
    enddo
  ...
  PROFIL_EVOLUTION: SELECT CASE(PROF_EVOL%OPT)
  CASE(PROF_EVOL_UNIF) ! flat deposit - uniform erosion (PU2017)
     Do i = 1,NbProfil
        minZb1 = INFINI
        minZb2 = INFINI
  ...
  CASE(PROF_EVOL_SIMP) ! uniform deposit - erosion
     CALL PROF_EVOL%UPDATE(DAZ)
  END SELECT PROFIL_EVOLUTION

Subroutine Sedimento (  &
  ...
  tau_option     ,  & ! formule de calcul de la contrainte
  Erreur         )
...
!      return
 ...
 ! Initialisation des variables aux points de la section en travers
    ...
    Do j = LimiteDepotG(i), LimiteDepotD(i)
      ...
      ! contrainte efficace avec un alpha = 2 (? a confirmer)
      ...
      ! TaE = Rho * Gpes * V*V / (Kt*Kt * Rh13) * (Kt/Kp)
      ! Engelund-Hansen prend en compte une contrainte totale et non efficace
      ! alpha = 0 dans ce cas (? a confirmer)
      TaE = Rho * Gpes * V*V / (Kt*Kt * Rh13)
      ! glute pour le cas d50 = 0, probleme lecture d50 dans fichier cas ?
      If (d50.GT.0.0) Then
        Qv  = 0.05_DOUBLE * sqrt(d50 / (Gpes * (RhoS / Rho - 1))) * &
              Kt * Kt * Rh13 / (Rho*Gpes) * &

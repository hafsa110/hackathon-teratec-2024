...
!***********************************************************************
   ...
   Boucle_profil : do iprof = 1 , size( Profil )
     ...
     IF(PROF_EVOL%OPT == PROF_EVOL_SIMP &
        .AND. Temps > TempsInitial .AND. condition_courlis) THEN
        CALL PROF_EVOL%PLANIM(IPROF, PROFILPLAN)
        varsed(iprof) = 0.0
        CYCLE Boucle_profil
     END IF
   ...
   my_cpt_planim = my_cpt_planim + 1  ! Increment of global variable to count number of "planimetrage" steps

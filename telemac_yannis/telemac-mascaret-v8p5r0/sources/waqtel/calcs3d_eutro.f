...
!
     ...
     &   ZPROP,T1,T2_1,T2_2,T2_3,T3,T4,T5,T6,T7,T8,T9,T10,T11,T12,
     &   UN,VN,DT)
!
!***********************************************************************
! WAQTEL   V8P5
...
!***********************************************************************
     ...
     &  O2NITRI,DEMBEN,MEXTINC,SECTODAY,IND_T,IND_S,WATSAL,
...
!
      ...
      TYPE(BIEF_OBJ)   , INTENT(INOUT) :: T10,T11,T12,T2_2,T2_3
!
!+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
!
!     LOCAL VARIABLES
...
!     CS IS STORED IN T2_1 ==> 2D TABLE
      CALL OS( 'X=0     ', X=T2_1)
!     HALF BOTTOM LAYER THICKNESS IS STORED IN T2_2 ==> 2D TABLE
      CALL OS( 'X=0     ', X=T2_2)
!     WATER DEPTH ABOVE NODE IS STORED IN T2_3 ==> 2D TABLE
      CALL OS( 'X=0     ', X=T2_3)
...
!
      IF(IND_T.EQ.0.AND.IND_S.EQ.0) THEN
        CALL SATUR_O2(O2SATU,FORMCS,WATTEMP,WATSAL,EPS)
        CALL OS('X=C     ', X=T2_1, C=O2SATU)
      ELSEIF(IND_S.EQ.0) THEN
        DO I=1,NPOIN2
          J=(NPLAN-1)*NPOIN2+I
          CALL SATUR_O2(T2_1%R(I),FORMCS,TN%ADR(IND_T)%P%R(J),WATSAL,
     &                  EPS)
        ENDDO
      ELSE
        DO I=1,NPOIN2
          J=(NPLAN-1)*NPOIN2+I
          CALL SATUR_O2(T2_1%R(I),FORMCS,TN%ADR(IND_T)%P%R(J),
     &                  TN%ADR(IND_S)%P%R(J),EPS)
        ENDDO
      ENDIF
!
...
!
      CALL OS('X=Y+Z   ', X=T1, Y=TN%ADR(IND_NH4)%P,
     &        Z=TN%ADR(IND_NO3)%P)
...
!     RESPIR*G1 IS STORED IN T5 FOR THE 8TH EQUATION
      G1 = WATTEMP/20.D0
      IF(IND_T.GT.0) THEN
        CALL OS('X=CY    ', X=T5, Y=TN%ADR(IND_T)%P,
     &                      C=UNSURVINGT*TRESPIR)
      ELSE
...
!
      DO I=1,NPOIN2
        J=(NPLAN-1)*NPOIN2+I
!       BOTTOM LAYER THICKNESS/2: STORED IN T2_2 ==> 2D TABLE
!       WATER DEPTH ABOVE NODE: STORED IN T2_3 ==> 2D TABLE
!       NOT SURFACE LAYER THICKNESS/2
!       T2_2%R(I) = 0.5D0*(ZPROP%R(J)-ZPROP%R(J-NPOIN2))
        T2_2%R(I) = 0.5D0*(ZPROP%R(I+NPOIN2)-ZPROP%R(I))
        T2_3%R(I) = ZPROP%R(J)-ZPROP%R(I)
      ENDDO
!
...
!     FROM V8P5:
!     6 FOLLOWING LINES TEMPORARILY COMMENTED IN 3D AS LONG AS A CORRECT
!     SOLUTION HAS NOT BEEN FOUND (SEDIMENTATION MAY HAVE BEEN DONE
!     LAYER BY LAYER OR SOMETHING ELSE BUT NOT FOR 1 SINGLE LAYER,
!     TO BE INVESTIGATED)
!      DO I=1,NPOIN2
!        J=(NPLAN-1)*NPOIN2+I
!!       - FPOR/H = WPOR[POR]/H
!        TEXP%ADR(IND_POR)%P%R(J) = TEXP%ADR(IND_POR)%P%R(J)
!     &                           - SECTODAY*WPOR*TN%ADR(IND_POR)%P%R(J)
!      ENDDO
!
...
!     &                           - SECTODAY*WNOR*TN%ADR(IND_NOR)%P%R(J)
!      ENDDO
!
...
!      CALL OS( 'X=X-Y   ', X=TEXP%ADR(IND_O2)%P, Y=T1               )
      ...
      DO I=1,NPOIN2
        J=(NPLAN-1)*NPOIN2+I
!       SURFACE FLUX: K2G4(CS-[O2])
        TEXP%ADR(IND_O2)%P%R(J) = TEXP%ADR(IND_O2)%P%R(J)
!       REAERATION ([O2]<CS) & DEGASSING ([O2]>CS) ALLOWED
     &    + SECTODAY*(K2%R(I)*T9%R(J)*(T2_1%R(I)-TN%ADR(IND_O2)%P%R(J)))
!       BOTTOM FLUX: -BEN/H
!       BEWARE O2 CAN BE NEGATIVE AT THE BOTTOM WITH A CONSTANT FLUX BEN
        IF(T2_3%R(I).GE.EPS) THEN
          TEXP%ADR(IND_O2)%P%R(I) = TEXP%ADR(IND_O2)%P%R(I)
     &                            - SECTODAY*T10%R(I)/T2_2%R(I)
        ENDIF
      ENDDO
!
      DO I=1,NPOIN3
...
!-----------------------------------------------------------------------
!
!     COMPUTE O2 SATURATION CONCENTRATION CS FOR POST-PROCESSING
!     AND SAVE IT IN T1 FOR WRITING IN 3D OR 2D RESULT FILE
!
      IF(IND_T.EQ.0.AND.IND_S.EQ.0) THEN
        CALL SATUR_O2(O2SATU,FORMCS,WATTEMP,WATSAL,EPS)
        CALL OS('X=C     ', X=T1, C=O2SATU)
      ELSEIF (IND_S.EQ.0) THEN
        DO I=1,NPOIN3
          CALL SATUR_O2(T1%R(I),FORMCS,TN%ADR(IND_T)%P%R(I),WATSAL,EPS)
        ENDDO
      ELSE
        DO I=1,NPOIN3
          CALL SATUR_O2(T1%R(I),FORMCS,TN%ADR(IND_T)%P%R(I),
     &                  TN%ADR(IND_S)%P%R(I),EPS)
        ENDDO
      ENDIF
!
!-----------------------------------------------------------------------

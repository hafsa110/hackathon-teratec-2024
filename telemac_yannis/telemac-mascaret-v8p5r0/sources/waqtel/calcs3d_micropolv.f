...
!                   *****************************
     & (NPOIN2,NPLAN,TN,TEXP,TIMP,ZPROP,CF,UN,VN,T2_1,T2_2,T2_3,T2_4,
     &  T3_1,T3_2,T3_3)
!
!***********************************************************************
! WAQTEL   V8P5
...
!
      ...
      TYPE(BIEF_OBJ)   , INTENT(INOUT) :: T3_1,T3_2,T3_3
!
!+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
!
!     LOCAL VARIABLES
...
!     WATER DEPTH: STORED IN T2_4 ==> 2D TABLE
      DO I=1,NPOIN2
        T2_3%R(I) = 0.5D0*(ZPROP%R(I+NPOIN2)-ZPROP%R(I))
        T2_4%R(I) = (ZPROP%R(I+NPOIN2*(NPLAN-1))-ZPROP%R(I))
      ENDDO
!
...
!     BED SOURCES
      DO I=1,NPOIN2
        IF(T2_4%R(I).GE.EPS) THEN
          TEXP%ADR(IND_SS)%P%R(I) = TEXP%ADR(IND_SS)%P%R(I)
     &                            - T2_2%R(I)/T2_3%R(I)
        ENDIF
      ENDDO
!
...
!
      DO I=1,NPOIN2
        IF(T2_4%R(I).GE.EPS) THEN
          T3_1%R(I) = RS%R(I)*TN%ADR(IND_CSF)%P%R(I)/T2_3%R(I)
     &              - T2_2%R(I)*TN%ADR(IND_CSS)%P%R(I)/T2_3%R(I)
        ELSE
          T3_1%R(I) = 0.D0
        ENDIF
      ENDDO
...
!       explicit part
        ...
        DO I=1,NPOIN2
          IF(T2_4%R(I).GE.EPS) THEN
            T3_1%R(I) = RS%R(I)*TN%ADR(IND_CSF2)%P%R(I)/T2_3%R(I) -
     &                  T2_2%R(I)*TN%ADR(IND_CSS2)%P%R(I)/T2_3%R(I)
          ELSE
            T3_1%R(I) = 0.D0
          ENDIF
        ENDDO

...
!
     ...
     &  PHOTO,RESP,TN,TEXP,TIMP,T31,T32,T21,T22,T23,HPROP,ZPROP,UN,VN)
!
!***********************************************************************
! WAQTEL   V8P5
!***********************************************************************
...
!
     ...
     &  IND_T,IND_O2,IND_OL,IND_NH4,IND_S,WATSAL
...
!
      ...
      TYPE(BIEF_OBJ)   , INTENT(INOUT) :: TEXP,TIMP,T31,T32,T21,T22,T23
!
!+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
!
!     LOCAL VARIABLES
...
!
      ...
      ELSEIF(IND_S.EQ.0) THEN
        DO I=1,NPOIN2
          J=(NPLAN-1)*NPOIN2+I
          CALL SATUR_O2(T21%R(I),FORMCS,TN%ADR(IND_T)%P%R(J),WATSAL,
     &                  EPS)
        ENDDO
      ELSE
        DO I=1,NPOIN2
          J=(NPLAN-1)*NPOIN2+I
          CALL SATUR_O2(T21%R(I),FORMCS,TN%ADR(IND_T)%P%R(J),
...
!     SURFACE SOURCES
      DO I=1,NPOIN2
        J=(NPLAN-1)*NPOIN2+I
!       BOTTOM LAYER THICKNESS/2: TEMPORARILY STORED IN T22 ==> 2D TABLE
!       WATER DEPTH ABOVE NODE: STORED IN T23 ==> 2D TABLE
!       NOT SURFACE LAYER THICKNESS/2
!       T22%R(I) = 0.5D0*(ZPROP%R(J)-ZPROP%R(J-NPOIN2))
        T22%R(I) = 0.5D0*(ZPROP%R(I+NPOIN2)-ZPROP%R(I))
        T23%R(I) = ZPROP%R(J)-ZPROP%R(I)
        TEXP%ADR(IND_O2)%P%R(J) = TEXP%ADR(IND_O2)%P%R(J)
!       REAERATION ([O2]<CS) & DEGASSING ([O2]>CS) ALLOWED
     &  + T31%R(J)*K2%R(I)*(T21%R(I)-TN%ADR(IND_O2)%P%R(J))
!       BOTTOM FLUX: -BEN/H
        IF(T23%R(I).GE.EPS) THEN
...
!     AND SAVE IT IN T31 FOR WRITING IN 3D OR 2D RESULT FILE
!
      IF(IND_T.EQ.0.AND.IND_S.EQ.0) THEN
        CALL SATUR_O2(O2SATU,FORMCS,WATTEMP,WATSAL,EPS)
        CALL OS('X=C     ', X=T31, C=O2SATU)
      ELSEIF (IND_S.EQ.0) THEN
        DO I=1,NPOIN3
          CALL SATUR_O2(T31%R(I),FORMCS,TN%ADR(IND_T)%P%R(I),WATSAL,EPS)
        ENDDO
      ELSE
        DO I=1,NPOIN3
          CALL SATUR_O2(T31%R(I),FORMCS,TN%ADR(IND_T)%P%R(I),

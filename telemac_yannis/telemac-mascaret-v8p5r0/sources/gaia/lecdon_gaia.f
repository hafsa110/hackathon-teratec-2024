...
!       ===
        ...
        ELSEIF(GAI_FILES(I)%TELNAME.EQ.'GAIRST') THEN
          GAIRST=I
!       TIME SERIES FILES
        ELSEIF(GAI_FILES(I)%TELNAME.EQ.'GAICOO') THEN
          GAICOO=I
        ELSEIF(GAI_FILES(I)%TELNAME.EQ.'GAIHI2') THEN
          GAIHI2=I
!
        ENDIF
      ENDDO
...
!
      ...
      SPINUP_TIME = MOTREA( ADRESS(2, 54) )
!
!     COHESIVE SEDIMENT
!     +++++++++++++++++
!
...
!
      ...
      OUTPUT_MOFAC = MOTLOG( ADRESS(3,  6) )
...
!     TIME SERIES COORDINATES FILE
      GAI_FILES(GAICOO)%NAME=MOTCAR( ADRESS(4,24) )
!     TIME SERIES FILES
...
!
!-----------------------------------------------------------------------
!      LIST OF NECESSARY VARIABLES FOR RESTART FILE
!
!     B
      SOREST(5)=.TRUE.
!     R: RIGID BED
...
!-----------------------------------------------------------------------
!
!     FORCE THE VARIABLES NECESSARY FOR RESTARTING TO BE IN THE RESTART
!     FILE
...
!     MORPHOLOGICAL FACTOR
      IF ((ABS(MOFAC - 1.D0).GT.1E-8).AND.BED_MODEL.NE.1) THEN
...
!
!     SOULSBY OR APSLEY&STANSBY SLOPE EFFECT : REQUIRES A THRESHOLD FORMULA
!
      IF(SLOPEFF.EQ.2.OR.SLOPEFF.EQ.3) THEN

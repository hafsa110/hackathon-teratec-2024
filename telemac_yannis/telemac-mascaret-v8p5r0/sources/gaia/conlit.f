...
!           AVOID OVERRIDING WITH SUSPENDED SEDIMENT TRANSPORT
              IF(GAI_FILES(GAILIQ)%NAME(1:1).NE.' ') THEN
                SOLDIS(IFRLIQ)=QGL(IFRLIQ,AT0)
              ENDIF
            ENDIF

            CALL DISIMP(SOLDIS(IFRLIQ),Q2BOR,NUMLIQ%I,IFRLIQ,
     &                  NSOLDIS,T5,T1,
...
! (NOT RELATED TO ONLY SANDS)
                      IF(NPROP.EQ.0) THEN
                        QBOR%ADR(I)%P%R(K)=
     &                        RATIO_SAND(ISAND,1,MESH%NBOR%I(K))*T1%R(K)
                      ELSE
                        QBOR%ADR(I)%P%R(K)=RATIO_DEBIMP(ISAND)*T1%R(K)
                      ENDIF
                    ENDIF
                  ENDDO
                ENDIF
            ...
            ELSE
              DO K=1,NPTFR
                IF(NUMLIQ%I(K).EQ.IFRLIQ.AND.
     &             LIQBOR%I(K).EQ.KENT) THEN
                  IF(.NOT.SEDCO(1)) QBOR%ADR(1)%P%R(K)=T1%R(K)
                ENDIF
              ENDDO
            ENDIF
          ENDIF

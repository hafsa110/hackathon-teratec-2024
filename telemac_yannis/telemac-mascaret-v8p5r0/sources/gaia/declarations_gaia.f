...
!!    In kg/s, for every class
!
      TYPE(BIEF_OBJ), TARGET :: QBOR
!
!>    Imposed solid transport at the boundary
!!    In kg/m/s, total, read in the boundary conditions file
!
...
! --------
!
!>    Surface roller energy  (m3/s2)
!
      TYPE(BIEF_OBJ), TARGET :: SURF_ENG
!
!>    Surface roller energy dissipation (m3/s3)
!
      TYPE(BIEF_OBJ), TARGET :: SURF_DIS
!
...
!>    Bed roughness from telemac at first time step
!
      TYPE(BIEF_OBJ), TARGET :: KS_T0
!
...
!
      LOGICAL :: CONV_WAVES
!
!>    Logical for output time according to morphological factor
!
...
!
      DOUBLE PRECISION :: PI
!
!>    Meyer-Peter and Mueller coefficient
!
...
!
!     >    Morphological factor: classical approach as
!!    described by Warner et al (2008), based on the
!!    multiplication of bedload fluxes (bedload),
!!    and erosion and deposition rates (suspension).
!!    When activated, the final simulation time
!!    will be equal to MOFAC x total simulation time
!!    provided by the user. In other words, one
!!    hydrodynamics step is done every MOFAC
!!    morphodynamic step.
!!    Special attention must be put as the application of
!!    the morphodynamics factor requires to distort
!!    the hydrodynamics forcing
!!    (for example the hydrograph at the entrance of
!!    the domain) with the same factor: accounting for
!!    this requirement is responsability of the end user.
!
...
!
!>    Time after which bed updating becomes active
!
      DOUBLE PRECISION :: SPINUP_TIME
...
!
      INTEGER, PARAMETER :: MAXLU_GAI = 48
!

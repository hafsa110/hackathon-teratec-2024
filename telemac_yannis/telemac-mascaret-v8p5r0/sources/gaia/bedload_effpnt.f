...
!
     ...
     &  TOB,XMVS,XMVE,DCLA,GRAV,UNSV2D,AC)
!
!***********************************************************************
! GAIA
!***********************************************************************
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
!
      USE INTERFACE_GAIA,EX_BEDLOAD_EFFPNT => BEDLOAD_EFFPNT
      USE BIEF
...
!     GLOBAL VARIABLES
      ...
      DOUBLE PRECISION, INTENT(IN)    :: XMVS, XMVE, GRAV, DCLA,AC
...
!
      ...
      ENDDO
!
!     3 : APSLEY AND STANSBY: TRANSPORT ON LARGE SLOPES JHR 2008 134(10)
!
      CASE (3)

      C = (XMVS-XMVE)*GRAV*DCLA
      TANPHI = TAN(PHISED*PI/180.D0)
      C3 = AC/TANPHI
      DO I = 1 , NPOIN
        INVCBETA=SQRT(1.D0+DZFDX%R(I)**2+DZFDY%R(I)**2)
        TT1 = TOB%R(I) / C
        AA = TT1*STETA%R(I)-C3*DZFDY%R(I)/INVCBETA**2
        BB = TT1*CTETA%R(I)-C3*DZFDX%R(I)/INVCBETA**2
        NORM=SQRT(AA**2+BB**2)
        NORM=MAX(SQRT(AA**2+BB**2),1.D-10)

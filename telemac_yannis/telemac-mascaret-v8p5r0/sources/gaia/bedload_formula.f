...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
!
      USE INTERFACE_GAIA,EX_BEDLOAD_FORMULA => BEDLOAD_FORMULA
      USE BIEF
      USE DECLARATIONS_GAIA, ONLY : SUSP_SAND,SLIDE
...
!     =====================================
      IF(ICF == 1) THEN
!
        CALL BEDLOAD_MEYER(TETAP,HIDING,HIDFAC,DENS,GRAV,DCLA,AC,
...
!     ===========================
      ELSEIF(ICF == 2) THEN
        CALL BEDLOAD_EINST(TETAP,NPOIN,DENS,GRAV,DCLA,DSTAR,QSC,
     &                         XMVS)
        DO I=1,NPOIN
...
!       BEWARE: DIFFERENCES
!               CALL BEDLOAD_ENGEL(TETAP,DENS,GRAV,DCLA,QSC)
!       BACK TO EARLIER VERSION OF BEDLOAD_ENGEL
        CALL BEDLOAD_ENGEL(TOB,CF,DENS,GRAV,DCLA,XMVE,T1,QSC,XMVS)
...
!RK -- DISCUSSION: There can be avalanching even if water level is lower HMIN
!     AVALANCHING AFTER APSLEY AND STANSBY 2008
      IF(SLIDE.EQ.2) THEN
        CALL BEDLOAD_AVALANCHE(QSC,RATIO_SAND,XMVS,T1,T2,CALFA,SALFA)
      ENDIF

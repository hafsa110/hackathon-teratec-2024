...
!
      USE BIEF
      USE INTERFACE_GAIA, EX_BEDLOAD_MAIN => BEDLOAD_MAIN
...
!         FOR COHESIVE SEDIMENT: ZERO BEDLOAD TRANSPORT RATE
        ...
        ENDIF
!
!       MORPHOLOGICAL FACTOR APPLIED ON QSC DIRECTLY TO TAKE INTO
!       ACCOUNT RIGID BED
!       AFTER EVOLUTION OF BED, QSC TAKE THE RIGHT VALUE
        DO IPOIN=1,NPOIN
          QSCL_C%ADR(I)%P%R(IPOIN)=QSCL_C%ADR(I)%P%R(IPOIN)*MOFAC
        ENDDO
!
      ENDDO
!
...
!
          ...
          TMP_RATIO = RATIO_SAND(K,1,1:NPOIN)
          CALL BEDLOAD_EVOL(S,COEFPN,CALFA_CL%ADR(I)%P,
     ...
     &                      TMP_RATIO,EVCL_MB%ADR(I)%P)
          ...
          !No evolution during spinup
          IF (AT0.LE.SPINUP_TIME) THEN
            CALL OS('X=0     ', EVCL_MB%ADR(K)%P)
          ENDIF
...
!
     ...
     &         EVCL_MB%ADR(K)%P%R(IPOIN) *VOLU2D%R(IPOIN)
        ...
        ENDIF
!
        CALL NESTOR_INTERFACE(2,123,XMVS0,XKV0(1),VOLU2D)
!
        IF(NSAND.GT.0) THEN
          T8%R = 0.D0
          DO IPOIN=1,NPOIN
...
!
      DO I=1,NSICLA
!
!       AFTER BED EVOLUTION, QSC TAKE ITS ORIGINAL VALUE
!       SEE BEFORE FOR MULTIPLICATION BY MOFAC
        DO IPOIN=1,NPOIN
          QSCL_C%ADR(I)%P%R(IPOIN)=QSCL_C%ADR(I)%P%R(IPOIN)/MOFAC
        ENDDO
!
...
!           IN BED1_SUSPENSION_ERODE WHERE MOFAC IS ALSO
!           TAKEN INTO ACCOUNT
            F_MUDB%ADR(I)%P%R(IPOIN)= -MUDB%ADR(I)%P%R(IPOIN)/DT
          ENDDO
        ENDIF
      ENDDO
!

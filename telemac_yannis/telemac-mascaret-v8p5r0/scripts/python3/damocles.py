...
def eficas_translation(ts_file, new_ts_file, lang):
    """
    Applying modification to the translation file for eficas
...
def run_damocles(exe_path, parame_file, log_file=''):
    """
       ...
       @param log_file Redirecting output to that file if present
    """
...
def gen_cata(code_name, exe_path, input_dict, input_dep,
    ...
    # Cannot check catalogue as you need eficas environment
    import importlib
...
#   <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    if args.modules == '':
        module_list = ['artemis', 'postel3d', 'stbtel', 'telemac2d',

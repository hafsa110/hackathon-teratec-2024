...
def check_python_rank_tags(py_file, options):
    ...
    # Checking ranks: it will run all ranks <= options.rank
...
def run_validation_python_mpi(cfg, options, report, xcpts):
    """
    Run validation for vnv Python scripts Normal mode
...
def run_validation_python_slurm(cfg, options, report, xcpts):
    ...
    for py_file in run_list_files:
        # Check if the file is in the failed files
...
def set_parser():
    """
    Defining parser for validate_telemac.py
...
- slurm: Will run validation in 'cluster' mode.
    ...
    parser.add_argument(
        ...
        help="Time after which the notebook will be killed if still running")
    parser.add_argument(
        "--notebook-update",
        dest="nb_update",
        action="store_true", default=False,
...
def config_corrections(options, cfgname):
    """
    Overwrite configuration with options arguments
...
def main():
    """
     @brief Main function of validate_telemac.py
    """
    # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    # ~~~~ Forces not to use any X Window backend for Jenkins ~~~~~~~~~~
    if options.vnv_post:

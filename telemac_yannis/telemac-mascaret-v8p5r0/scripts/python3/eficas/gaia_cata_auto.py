...
#   -----------------------------------
    TIME_SERIES_FILE = SIMP(statut ='o',
#   -----------------------------------
        typ = ('Fichier','All Files (*)','Sauvegarde'),
        defaut = '',
...
#   -----------------------------------
    TIME_SERIES_FILE_FORMAT = SIMP(statut ='o',
#   -----------------------------------
        typ = 'TXM',
        into = ['SERAFIN','SERAFIND','MED'],
        defaut = 'SERAFIN',
        fr = """Format du \telkey{FICHIER DES SERIES TEMPORELLES}.
Les valeurs possibles sont :
\begin{itemize}
\item SERAFIN : format standard simple precision pour \tel ;
\item SERAFIND: format standard double precision pour \tel ;
\item MED     : format MED double precision base sur HDF5.
\end{itemize}""",
        ang = """Format of the \telkey{TIME SERIES FILE}. Possible choices are:
\begin{itemize}
\item SERAFIN : classical single precision format in \tel,
\item SERAFIND: classical double precision format in \tel,
\item MED     : MED double precision format based on HDF5.
...
\telkey{FICHIER DES SERIES TEMPORELLES}.""",
...
#   -----------------------------------
    SPINUP_TIME_FOR_BED_UPDATING = SIMP(statut ='f',
#   -----------------------------------
        typ = 'R', min=0, max='**',
        defaut = [0.],
        fr = """Temps à partir duquel se font les mises à jour du lit""",
        ang = """Time in seconds after which bed updates are taken into account""",
    ),
#   -----------------------------------
...
\telkey{FICHIER PRECEDENT SEDIMENTOLOGIQUE} du fichier des parametres
...
souhaite parfaite (le mot-cle
\telkey{FICHIER PRECEDENT SEDIMENTOLOGIQUE} est alors utilise).
Le \telkey{FORMAT DU FICHIER POUR SUITE} et le
\telkey{FORMAT DU FICHIER PRECEDENT SEDIMENTOLOGIQUE} doivent alors etre
mis a ''SERAFIND'' ou ''MED''.""",
...
\telkey{PREVIOUS SEDIMENTOLOGICAL COMPUTATION FILE} is then used).
The \telkey{RESTART FILE FORMAT} and the
\telkey{PREVIOUS SEDIMENTOLOGICAL COMPUTATION FILE FORMAT} have to be
set with ''SERAFIND'' or ''MED''.""",
        ),
#       -----------------------------------
...
#       -----------------------------------
            ...
            ang = """Amplification coefficient for bed evolution""",
        ),
#       -----------------------------------
        OUTPUT_TIME_ACCORDING_TO_MORPHOLOGICAL_FACTOR = SIMP(statut ='f',
#       -----------------------------------
            typ = bool,
            defaut = False,
            fr = """Si oui le temps ecrit dans le fichier resultat de \gaia correspond
au temps de l''hydrodynamique multiplié par le facteur morphologique.""",
            ang = """If yes the output time in the \gaia result file will be equal
to the hydrodynamic time multiplied by the morphological factor.""",
        ),
#       -----------------------------------
...
#           -----------------------------------
                ...
                fr = """Choix possible:
\begin{itemize}
\item 1 : formule de Koch et Flokstra, modifie le transport solide.
 Mot cle associe : \telkey{BETA}.
\item 2 : formule de Soulsby, modifie la contrainte seuil, ne peut
 donc etre utilisee que avec une formule a seuil.
 Mot cle associe : \telkey{ANGLE DE FROTTEMENT DU SEDIMENT}.
\item 3 : formule de Apsley and Stansby, modifie la contrainte seuil,
ne peut donc etre utilisee que avec une formule a seuil.
Mot cle associe : \telkey{ANGLE DE FROTTEMENT DU SEDIMENT}.
\item 4 : formule de Apsley and Stansby, modifie la contrainte seuil,
ne peut donc etre utilisee que avec une formule a seuil et avalanche
effect. Mot cle associe : \telkey{ANGLE DE FROTTEMENT DU SEDIMENT}
\end{itemize}""",
                ang = """Possible choices:
\begin{itemize}
\item 1: formula of Koch et Flokstra, modification of bed load
linked keyword : \telkey{BETA}.
\item 2 : formula of Soulsby, modification critical shear stress,
can only be used with a threshold fomula.
Linked keyword : \telkey{FRICTION ANGLE OF THE SEDIMENT}
\item 3 : formula of Apsley and Stansby, modification of critical
shear stress and the bed load.
Linked keyword: \telkey{FRICTION ANGLE OF THE SEDIMENT}
\item 4 : formula of Apsley and Stansby, modification of critical
shear stress and the bed load and additionally avalanching.
Linked keyword: ANGLE DE FROTTEMENT DU SEDIMENT
\end{itemize}""",
            ),
#           -----------------------------------
...
#           -----------------------------------
                typ = 'I',
                defaut = 0,
                fr = """two choices of formulas, both take \telkey{FRICTION ANGLE OF THE
SEDIMENT} into account:
\begin{itemize}
\item 0: No sliding
\item 1: Sliding classic
\item 2: Avalanching according Apsley & Stansby
\end{itemize}""",
                ang = """Two choices of formulas, both take \telkey{FRICTION ANGLE OF THE
SEDIMENT} into account:
\begin{itemize}
\item 0: No sliding
\item 1: Sliding classic
\item 2: Avalanching according Apsley and Stansby
\end{itemize}""",
            ),
        ),
    ),
...
ERIA works only in 2D. The order of the chosen scheme must follow
...
\begin{itemize}
\item 0 : Formule définie par l''utilisateur
\item 1 : Whitehouse et al. (2000)
\item 2 : Winterwerp (1999)
\end{itemize}""",
            ang = """Type of hindered settling:
\begin{itemize}
\item 0 : User defined formula
\item 1 : Whitehouse et al. (2000)
\item 2 : Winterwerp (1999)
\end{itemize}""",
        ),
...
#       -----------------------------------
            typ = bool,
            defaut = False,
            fr = """Decide si la formulation entravee doit etre utilisee. Uniquement en 3D.
pour calculer la vitesse de chute pour la vase.""",
            ang = """Decides if hindered formulation is to be used to
compute settling velocity for mud. Only in 3D.""",
        ),
#       -----------------------------------

...
#           -----------------------------------
            TIME_SERIES_COORDINATES_FILE = SIMP(statut ='o',
#           -----------------------------------
                typ = ('Fichier','All Files (*)'),
                defaut = '',
                fr = """Nom du fichier contenant les coordonnees des points et les intervalles
de temps ou les series temporelles sont extraites dans le
\telkey{FICHIER DES SERIES TEMPORELLES} (2D ou 3D).""",
                ang = """Name of the file containing points coordinates and periods of time
where time series are extracted in the \telkey{TIME SERIES FILE}
(2D or 3D).""",
            ),
        ),
    ),
...
\item CSO2 : concentration de saturation en oxygene (mgO2/l) ;
\item O2SAT : pourcentage de saturation en oxygene (\%).
\end{itemize}""",
...
\item CSO2: oxygen saturation concentration (mgO2/l),
\item O2SAT: percentage of oxygen saturation (\%).
\end{itemize}""",
            ),
#           -----------------------------------
...
\item CSO2 : concentration de saturation en oxygene moyenne (mgO2/l) ;
\item O2SAT : pourcentage de saturation en oxygene moyen (\%).
\end{itemize}""",
...
\item CSO2: depth averaged oxygen saturation concentration (mgO2/l),
\item O2SAT: depth averaged percentage of oxygen saturation (\%).
\end{itemize}""",
            ),
#           -----------------------------------
...
#           -----------------------------------
            ED_TIME_SERIES_FILE = SIMP(statut ='o',
#           -----------------------------------
                typ = ('Fichier','All Files (*)','Sauvegarde'),
                defaut = '',
                fr = """Nom du fichier dans lequel les series temporelles 2D sont ecrites
aux points definis dans le
\telkey{FICHIER DE COORDONNEES DES SERIES TEMPORELLES}.""",
                ang = """Name of the file into which 2D time series are written at the points
defined in the \telkey{TIME SERIES COORDINATES FILE}.""",
            ),
#           -----------------------------------
            ED_TIME_SERIES_FILE_FORMAT = SIMP(statut ='o',
#           -----------------------------------
                typ = 'TXM',
                into = ['SERAFIN','SERAFIND','MED'],
                defaut = 'SERAFIN',
                fr = """Format du \telkey{FICHIER DES SERIES TEMPORELLES 2D}.
Les valeurs possibles sont :
\begin{itemize}
\item SERAFIN : format standard simple precision pour \tel ;
\item SERAFIND: format standard double precision pour \tel ;
\item MED     : format MED double precision base sur HDF5.
\end{itemize}""",
                ang = """Format of the \telkey{2D TIME SERIES FILE}. Possible choices are:
\begin{itemize}
\item SERAFIN : classical single precision format in \tel,
\item SERAFIND: classical double precision format in \tel,
\item MED     : MED double precision format based on HDF5.
...
#           -----------------------------------
            RD_TIME_SERIES_FILE = SIMP(statut ='o',
#           -----------------------------------
                typ = ('Fichier','All Files (*)','Sauvegarde'),
                defaut = '',
                fr = """Nom du fichier dans lequel les series temporelles 3D sont ecrites
aux points definis dans le
\telkey{FICHIER DE COORDONNEES DES SERIES TEMPORELLES}.""",
                ang = """Name of the file into which 3D time series are written at the points
defined in the \telkey{TIME SERIES COORDINATES FILE}.""",
            ),
#           -----------------------------------
            RD_TIME_SERIES_FILE_FORMAT = SIMP(statut ='o',
#           -----------------------------------
                typ = 'TXM',
                into = ['SERAFIN','SERAFIND','MED'],
                defaut = 'SERAFIN',
                fr = """Format du \telkey{FICHIER DES SERIES TEMPORELLES 3D}.
Les valeurs possibles sont :
\begin{itemize}
\item SERAFIN : format standard simple precision pour \tel ;
\item SERAFIND: format standard double precision pour \tel ;
\item MED     : format MED double precision base sur HDF5.
\end{itemize}""",
                ang = """Format of the \telkey{3D TIME SERIES FILE}. Possible choices are:
\begin{itemize}
\item SERAFIN : classical single precision format in \tel,
\item SERAFIND: classical double precision format in \tel,
\item MED     : MED double precision format based on HDF5.
...
#           -----------------------------------
                typ = ('Fichier','All Files (*)'),
                defaut = '',
                fr = """Fichier des sections de controle, partitionne.""",
...
#       -----------------------------------
        INITIAL_TIME = SIMP(statut ='o',
#       -----------------------------------
            typ = 'R',
            defaut = 0.,
            fr = """Temps initial de la simulation (en secondes).""",
            ang = """Initial time of the computation (in seconds).""",
        ),
    ),
#   -----------------------------------
...
#   -----------------------------------
    MINIMUM_DISTANCE_BETWEEN_PLANES_CLOSE_TO_THE_BOTTOM = SIMP(statut ='f',
#   -----------------------------------
        typ = 'R',
        defaut = 0.2,
...
#   -----------------------------------
    MINIMUM_DISTANCE_BETWEEN_PLANES_CLOSE_TO_THE_FREE_SURFACE = SIMP(statut ='f',
#   -----------------------------------
        typ = 'R',
        defaut = 0.2,
...
#   -----------------------------------
    THRESHOLD_HEIGHT_BEFORE_CRUSHED_ELEMENTS = SIMP(statut ='f',
#   -----------------------------------
        typ = 'R',
        defaut = 0.,
...
En octas dans \waqtel ou diziemes dans \khione.""",
                ang = """Gives the value of cloud cover when it is constant
in time and space.
In octas for \waqtel or tenths for \khione.""",
            ),
#           -----------------------------------
...
#       -----------------------------------
        OPTION_FOR_SOFT_BOUNDARIES = SIMP(statut ='f',
#       -----------------------------------
            typ = 'TXM', min=0, max='**',
            into = ["Not a soft boundary","Method 1. Proportional to the speed","Method 2. Proportional to the square of speed"],
            fr = """On donne un entier par frontiere liquide.
Les choix possibles sont :
\begin{itemize}
\item 0 : Pas une frontiere douce ;
\item 1 : Methode 1. Proportionnel a la vitesse ;
\item 2 : Methode 2. Proportionnel au carre de la vitesse.
\end{itemize}""",
            ang = """One integer per liquid boundary is given.
Possible choices are:
\begin{itemize}
\item 0: Not a soft boundary ,
\item 1: Method 1. Proportional to the speed,
\item 2: Method 2. Proportional to the square of speed.
\end{itemize}""",
        ),
#       -----------------------------------
        COEFFICIENT_FOR_SOFT_BOUNDARIES = SIMP(statut ='f',
#       -----------------------------------
            typ = 'R', min=0, max='**',
            fr = """Valeurs des coefficients pour chaque frontiere douce.""",
            ang = """Values of the coefficients for each soft boundary.""",
        ),
    ),
#   -----------------------------------
...
#               -----------------------------------
                    typ = 'R',
                    defaut = 1.,
                    fr = """Fixe la valeur du coefficient d implicitation pour l etape de
diffusion.""",
...
\telkey{OPTION DE TRAITEMENT DES BANCS DECOUVRANTS} = 2 i.e.
gel des elements decouvrants (zones de bancs decouvrants sont masquees)
ou avec \telkey{ELEMENTS MASQUES PAR L''UTILISATEUR} = OUI.""",
            ang = """Sets the minimum water depth value $H$.
Only used with \telkey{OPTION FOR THE TREATMENT OF TIDAL FLATS} = 2
i.e. dry elements are frozen (tidal flats area are masked)
or with \telkey{ELEMENTS MASKED BY USER} = YES.""",
        ),
    ),
#   -----------------------------------
    TIDES = FACT(statut='o',
...
prise en compte pendant le transport de particules.""",
                ang = """For particles: drogues, oil spills.
...
# -----------------------------------------------------------------------
#   -----------------------------------
    VELOCITIES_IN_BINARY_DATABASE_2_FOR_TIDE = SIMP(statut ='f',
#   -----------------------------------
        typ = bool,
        defaut = False,
        fr = """Indique si la \telkey{BASE BINAIRE 2 DE DONNEES DE MAREE} contient
des vitesses ou circulations (h fois vitesses).
Pour les solutions de l OSU, laisser a la valeur par defaut NON
(circulation).
Pour HAMTIDE, choisir OUI.""",
        ang = """Indicates if \telkey{BINARY DATABASE 2 FOR TIDE} contains
velocities or circulations (h times velocities).
For OSU solutions, let default value NO (circulation).
For HAMTIDE, choose YES.""",
    ),
#   -----------------------------------
    ATMOSPHERIC_PRESSURE_AT_TIDAL_BOUNDARIES = SIMP(statut ='f',
#   -----------------------------------
        typ = bool,
        defaut = False,
        fr = """Prise en compte de la pression atmospherique au niveau de l imposition
de maree.
Methode du barometre inverse pour le calcul du decalage.""",
        ang = """Take in account atmospheric pressure at tidal boundary conditions.
Inverted barometer method used to compute the shift.""",
    ),
)
# -----------------------------------------------------------------------
...
#   -----------------------------------
        typ = 'TXM',
        into = ['','TOMAWAC','TOMAWACT3D','TOMAWAC2','TOMAWACT3D2','WAQTEL','DELWAQ','GAIA','KHIONE'],
        defaut = '',

...
from utils.files import (diff_text_files, get_file_content, is_newer,
                         put_file_content)
...
def clean_spaces(istr):
    """
    Remove double-spaces, trailing spaces and spaces at the beginning of a line
...
def clean_quotes(istr):
    ...
    while 1:
        ...
        if ostr == istr:
            break
    # ~~> Deal with double quotes (replace by single quotes)
    while 1:
        ostr = istr
        ...
        if ostr == istr:
            break
    # ~~> Remove the internal apostrophes
        ...
        if not ignoreblock:
            ...
            if proc:
                ...
                if proc.group('name') and DEBUG:
                    print('No name at TYPE END of TYPE ' + name)
            core.pop(0)
            headline = True
            continue
...
def parse_principal_wrap(lines):
    """
    Parse the principal / high level structure of a Fortran file
    @param lines (string) of Fortran code to be processed
    @return (list of string) as a split of the top level of the Fortran
        file (excluding declarations), the main name, its arguments and return,
        the interfaces if present and the main code
    """
    ...
    if proc:
        ...
        if proc:
            ...
            # ~~> Account for empty MODULE (including only INTERFACE and
            # CONTAINS)
...
def parse_principal_main(lines, who, typ, name, args, resu):
    ...
    @return (list of string) as a split between the top part of the Fortran
        file (including declarations), the main name, its arguments and return,
        the interfaces if present and the main code
    """
    core = []
    core.extend(lines)
...
def scan_enunciation(name):
    """
    Display differentiating between name and name.cmdf
...
def filter_principal_wrap_names(u_names, s_files):
    """
    Identify the name of function associated with files
...
def trim_tree(name, lname, lst, rebuild, maksystel):
    ...
    Scan the trre of FORTRAN calls to populate who is "up" and who is "dw"
        (caller and called). Create the order of dependency of the libraries.
    ...
    #
    # ~~: libdws
    #  the list of libs touched by name, including lname, but not necessarily

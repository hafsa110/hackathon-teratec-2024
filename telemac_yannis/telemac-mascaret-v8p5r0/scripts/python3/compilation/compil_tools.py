...
def compile_mascaret_dependencies(rebuild, homeres, verbose):
    ...
    @param verbose (boolean) If true print the compilation command
    """
    ...
    for src_name in src_names:
        ...
        if is_newer(obj_path, src_path) and rebuild < 2:
...
def create_obj_files(oname, oprog, odict, mes, tasks, bypass, homeres, verbose):
    ...
    @returns True if nothing was done (i.e. file up to date)
    """
    cfgname = CFGS.cfgname
    cfg = CFGS.configs[cfgname]
...
def create_lib_files(lname, lmdul, lprog, mprog, mes, tasks,
    ...
    # - /!\ hopefully, the directory exists
    if lmdul == lib_name:
    ...
    if path.exists(lib_file):
        if 0 < cfg['COMPILER']['REBUILD'] < 2:
            remove(lib_file)
        else:
            refresh = False
            for obj in obj_files.split():
                refresh = refresh or is_newer(obj, lib_file) or \
                    not path.exists(obj)
            if refresh:
                remove(lib_file)
            else:
...
def create_exe_files(ename, emdul, eprog, mes, bypass, homeres, verbose):
    ...
    for pth, obj, lib in homeres[ename]['add']:
        ...
        if lib == emdul and obj_name  not in obj_files.split():
            ...
            if not path.exists(obj_file):
                raise TelemacException(f'Object missing:\n        {obj_file}')
    ...
    if path.exists(exe_file):
        if 0 < cfg['COMPILER']['REBUILD'] < 3:
            remove(exe_file)
        else:
            refresh = False
            for obj_file in obj_files.split():
                refresh = refresh or is_newer(obj_file, exe_file)
            for lib in lib_files.split():
                # Only checks the telemac libraries
                if lib.find(path.join(cfg['root'], 'builds',
                                      cfgname, 'lib')) != -1:
                    refresh = refresh or is_newer(lib, exe_file)
            if refresh:
                remove(exe_file)
            else:
                return True
...
def create_pyd_files(yname, yfile, ymdul, yprog, mes, bypass, homeres, verbose):
    ...
    if path.exists(pyd_file):
        if 0 < cfg['COMPILER']['REBUILD'] < 3:
            ...
            refresh = False
            for obj_file in obj_files.split():
                refresh = refresh or is_newer(obj_file, pyd_file)
            for lib in lib_files.split():
                # Only checks the telemac libraries
                if l.find(path.join(cfg['root'], 'builds',
                                    cfgname, 'lib')) != -1:
                    refresh = refresh or is_newer(lib, pyd_file)
            if refresh:
                remove(pyd_file)
            else:
                return True
...
def get_api_incs_flags():
    """
    Returns the string for incs_flags for api
...
def get_api_ld_flags(api_name):
    """
    Returns the string for ld_flags for api

    @param api_name Name of the API (hermes or api)
...
def copy_src_api(api_dir, src_list, src_dir):
    """
    Copy sources in src_list from src_dir to the API folder and convert them to
    Fortran 90. Return the list of sources as a string.

    @param api_dir (str) Path to the API directory
    @param src_list (str) List of files to copy
    @param src_dir (str) Path of the sources

    @return (str) The list of sources for F2PY
    """
    sources = ''
    for src in src_list:
...
def copy_lib_api(api_dir):
    """
    Copy libraries to the API folder.

    @param api_dir (str) Path to the API directory
    """
    cfg = CFGS.configs[CFGS.cfgname]
    lib_dir = path.join(cfg['root'], 'builds', CFGS.cfgname, 'lib')
    dyn_ext = cfg['sfx_lib']

    for lib in LIST_LIBS['api']:
        lib_name = lib.split('|')[-1]
        lib_name_tel = f'lib{lib_name}4api{dyn_ext}'
        # Mascaret is not named the same
        if lib == 'mascaret':
            lib_name_tel = f'lib{lib_name}{dyn_ext}'
            if not path.exists(path.join(lib_dir, lib_name_tel)):
                continue
        if lib == 'api':
            lib_name_tel = f'libapi{dyn_ext}'

        # Remove the file if its exists to avoid potential race conditions
        api_lib_path = path.join(api_dir, 'lib', lib_name_tel)
        if path.exists(api_lib_path):
            remove(api_lib_path)

        shutil.copy(path.join(lib_dir, lib_name_tel), api_lib_path)
...
def generate_api():
    ...
    # Copying sources for t3d, t2d, art and wac
    src_list = []
...
def compile_api_f2py(name, api_dir, source_list, skip_source, \
    ...
    displayed
    @param f2py_opt Additional options passed to F2PY (option --opt)
    """
    ...
    try:
        ...
        raise TelemacException(\
            f'Error during the first part of F2PY: {exception.returncode}\n'
            f'{exception.output.decode("utf-8")}')
    ...
    try:
        ...
        raise TelemacException(\
            f'Error during the second part of F2PY: {exception.returncode}\n'
            f'{exception.output.decode("utf-8")}')
        ...
        for item in sorted(cmdf_files[mod]):
            ...
            for lib in maksystel['deps']:
                ...
                for fli in cmdf_files[mod][item][lib]['files']:
                    ...
                    if is_newer(obj_name, src_name) and rebuild < 2:

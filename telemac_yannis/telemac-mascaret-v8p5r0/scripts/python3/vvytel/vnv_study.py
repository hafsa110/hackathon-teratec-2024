...
class AbstractVnvStudy(ABC):
    ...
    def _run_case(self, name, study):
        ...
        if study.code_name == 'mascaret':
            ...
            # Checks if the steering file is newer than the output file
            if path.exists(ori_cas_file) and is_newer(cas_file, out_file):
                run = True
            else:
                # Checks if any input file is newer than the output file
                for key in study.cas.in_files:
                    ffile = study.cas.get(key)
                    vnv_file = path.join(vnv_working_dir, ffile)
                    if is_newer(vnv_file, out_file):
                        run = True
                        break

        if run:
            print(f"  ~> {name}: running on {study.ncsize} cores")
            if self.listing:

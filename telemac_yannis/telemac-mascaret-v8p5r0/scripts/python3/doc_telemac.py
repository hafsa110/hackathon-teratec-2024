...
\end{longtable}""")
    ...
    for module in ['t2d', 't3d', 'art', 'wac']:

        content = ''
        if api_avail:
            print(" ~> Getting var for", module)
            # Using direct access to the API (no need to instantiate)
...
def compiletex(texfile, version, verbose):
    ...
    if verbose:
        tex_cmd = "openout_any=a pdflatex --jobname={tex}_{version} {tex}.tex"\
...
def create_case_list_file(doc_dir, val_dir, cfg_val, cleanup):
    ...
    @param cleanup If yes clean up the temporary files instead
...
def generate_ref_from_dict(exe_path, dictionary, latex_file, lng,
    ...
    @param latex_file (string) Name of the output latex file that will
                         contain the reference manual
    @param lng (int) Language for the reference manual
                 1: French
                 2: English
    @param cleanup (boolean) If True removing generated files
...
def generate_notebook_html(doc_dir, notebook_dir, verbose):
    ...
    # Running conversion in notebook folder
...
# <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
# ~~~~ Compile the validation documentation
    ...
    for code_name in module_list:
        ...
        if options.validation or doall:
            if code_name not in ['telapy', 'coupling']:

...
def wfs2xml(xml_file, year, bbox):
    ...
    wfs11 = WebFeatureService('http://wxs.ign.fr/clc/geoportail/' +
...
def get_typename(year):
    ...
    if year == 2012:
        ...
        typename = "LANDCOVER.CLC18_FR:clc18_fr"
    else:
        raise Exception(f'Unknown year: {year}\nHandled years: 2012 and 2018')
    ...
    for child in root:
        if child.tag == '{http://www.opengis.net/gml}featureMember':
            ...
            for zone_elem in zone:
                ...
                # print(zone_elem.tag)
                if re.match(
                    r'{https://datastore.wxs.ign.fr/datastore/' + tag +
                    r'}code_\d+', zone_elem.tag
            	):
...
def read_corresp_table(year, table_name=None):
    ...
    with open(tab_file, 'r') as f:
        ...
        for line in f.readlines():
            match = re.match(
            	r'"(?P<name>[^"]+)"\s+(?P<value>[^ ]+)\s+(?P<ref>[^ ]+)\s+(?P<code>[^ ]+)',
            	line
            )

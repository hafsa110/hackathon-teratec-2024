...
def subdivide_mesh4(ikle, meshx, meshy):
    ...
    jkle = np.zeros((ielem*4, 3), dtype=int)
    # you add one point on every edges
    meshj = np.zeros((iedge, 2), dtype=int)
    # n_o 3-node interpolation not necessary
    meshk = np.array([], dtype=int)
    ...
    neighbours = Triangulation(meshx, meshy, jkle).get_cpp_triangulation()\
                                                  .get_neighbors()
    jpobo = np.zeros(ipoin+iedge, int)
...
def subdivide_mesh3(ikle, meshx, meshy):
    ...
    jkle = np.zeros((ielem*3+isbnd, 3), dtype=int)
    # you add one point in the middle of every triangle
    meshk = np.zeros((ielem, 3), dtype=int)
    # you add one point on every boundary edges
    meshj = np.zeros((isbnd, 2), dtype=int)
    ...
    neighbours = Triangulation(meshx, meshy, jkle).get_cpp_triangulation()\
                                                  .get_neighbors()
    # ~~> Reset ipobo while you are at it
    jpobo = np.zeros(ipoin+ielem+isbnd, int)
...
def slice_mesh(polyline, ikle, meshx, meshy, tree=None):
    ...
    for i in range(len(polyline)-1):
        ...
        # around the polyline
        halo = np.zeros((nbpoints, nbneighs), dtype=int)
...
def tessellate_poly(i2s, debug=True):
    ...
    pol_areas = np.zeros(len(i2s.poly), dtype=float)
    if debug:
    ...
    pol_edges = []
    pol_nbres = np.zeros(i2s.npoin, dtype=int)
    cpt = 0
    if debug:
        pbar = ProgressBar(maxval=i2s.npoin).start()
    values = np.arange(len(i2s.poly), dtype=int)[np.argsort(pol_areas)][::-1]
    for ipoly in values:
        edges = []
        # number the edges
        ...
        if i2s.type[ipoly] == 0:
            edges = np.arange(len(i2s.poly[ipoly]), dtype=int) + cpt
        elif i2s.type[ipoly] == 1:
            edges = np.arange(len(i2s.poly[ipoly])+1, dtype=int) + cpt
            edges[-1] = edges[0]
    ...
    tri_mask = np.arange(len(ikle2), dtype=int)\
    ...
    if debug:
        print('     +> set ipobo')
    ipob2 = np.arange(i2s.npoin, dtype=int) + 1
...
def cross_check_boundaries(meshx, meshy, ikle2, ipob0, debug=True):
    ...
    sout = - np.ones(len(crosspts), dtype=int)
...
def get_ipobo(meshx, meshy, ikle2, debug=True):
    ...
    if debug:
        print('        - set')
    ipob2 = np.zeros(len(meshx), dtype=int)
    iptfr = 0
...
def show_boundary_nodes(meshx, meshy, ikle2, debug=True):
    ...
    ipob0 = np.zeros(len(meshx), dtype=int)
    pbar = ProgressBar(maxval=len(ikle2)).start()
    for i in range(len(ikle2)):
        if tri_neigh[i, 0] < 0:
            ipob0[ikle2[i][0]] += 1
...
def show_node_connections(meshx, meshy, ikle2, debug=True):
    ...
    ipob1 = np.zeros(len(meshx), dtype=int)
...
def remove_extra_nodes(meshx, meshy, ikle2, debug=True):
    ...
    likle = - np.ones_like(ikle2, dtype=int)
    if debug:
...
def map_thin_plate_spline(meshx, meshy, bathx, bathy, bathz, npoin,
    ...
    mask = np.zeros(npoin, dtype=int)

    varsor = np.zeros(len(meshx), dtype=float)

    if debug:
        print('     +> thin plate spline mapping')
        pbar = ProgressBar(maxval=len(meshx)).start()
    mat_a = np.zeros((npoin+3, npoin+3), dtype=float)
    row = np.zeros(npoin+3, dtype=float)
    for j in range(len(meshx)): # TODO: parallel implementation
        # ~~> subset of points
...
# ~~~~ Reads code name ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    varsor = np.zeros(len(slf.meshx), dtype=float)
    if options.mappping:

...
def alter(input_file, output_file,
    ...
    for i, (out_name, _) in enumerate(out_vars):
        ...
        for itime in out_records:
            if rotate is not None and 'VELOCITY' in out_name: 
                vitx = inres.get_data_value("VELOCITY U", itime)
                vity = inres.get_data_value("VELOCITY V", itime)
                vitrotx = np.cos(angle)*vitx - np.sin(angle)*vity
                vitroty = np.sin(angle)*vitx + np.cos(angle)*vity
            ...
            if rotate is not None and out_name == "VELOCITY U":
                value = vitrotx
            if rotate is not None and out_name == "VELOCITY V":
                value = vitroty

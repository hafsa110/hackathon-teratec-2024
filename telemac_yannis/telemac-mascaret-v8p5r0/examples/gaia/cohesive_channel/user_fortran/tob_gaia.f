...
!
      USE BIEF
      USE DECLARATIONS_GAIA, ONLY: NSAND, ES, LT, KS_T0
...
!
      INTEGER                     :: I
      DOUBLE PRECISION            :: A,B,C, HCLIP,KSMAX,TIERS
!
...
!
        IF(KSCALC) THEN
!
!     Saving the ks_tel at the first time step
          IF(LT.EQ.0) CALL OS('X=Y     ', X=KS_T0, Y=KS_TEL)
!
...
!     If there is sediment at the bottom, the dune roughness is sent to telemac
!     otherwise the initial telemac value is taken
          DO I = 1,NPOIN
            IF(ES(I,1).GT.0.D0) THEN
              KS_TEL%R(I) = KS%R(I)
            ELSE
              KS_TEL%R(I) = KS_T0%R(I)
            ENDIF
          ENDDO
        ELSE
!
...
! TOB=0 IF H_TEL < HMIN  (NOT USE HN BECAUSE HN = HMIN IN THIS CASE)
          IF(H_TEL%R(I).GE.HMIN)THEN
            TOB%R(I) = XMVE*UNORM%R(I)**2*GRAV
     &                 /85.D0**2/H_TEL%R(I)**TIERS
          ELSE
            TOB%R(I) = 0.D0
          ENDIF
        ENDDO

...
!
      USE BIEF
      USE DECLARATIONS_TELEMAC3D, ONLY: KARMAN, PRANDTL, FICT, IELM2H,
     &                                  MESH2D
!
      USE INTERFACE_TELEMAC3D, EX_ERODNC => ERODNC
      USE INTERFACE_GAIA, ONLY:SUSPENSION_FREDSOE, SUSPENSION_VANRIJN
!
...
!
!     ALLOCATE A DUMMY RATIO_TOCE TO USE THE GAIA SUBROUTINES IN PLACE
...
!
      IF(S3D_ICQ.EQ.1) THEN
        ...
        CALL SUSPENSION_FREDSOE(S3D_DMOY%R(1), TOB, NPOIN2, GRAV, RHO0,
     &                          S3D_RHOS, S3D_AC, S3D_CREF, RATIO_TOCE)
        CALL OS('X=CY    ', X=ZREF, Y=S3D_DMOY, C=2.D0)
      ELSEIF(S3D_ICQ.EQ.3) THEN
        CALL OS('X=CY    ', X=ZREF, Y=RUGOF, C=0.5D0)
        CALL SUSPENSION_VANRIJN(S3D_DMOY%R(1), TOB, NPOIN2, GRAV, RHO0,
     &                          S3D_RHOS, 1.D-06, 1.D-06, S3D_AC,
     &                          S3D_CREF, ZREF, RATIO_TOCE)
      ENDIF
!

...
"""
Validation script for Soni_trapeze
"""
...
class VnvStudy(AbstractVnvStudy):
    ...
    def _post(self):
        ...
        for kernel in TESTED_KERNELS:
            ...
            ax1.set_yticks(np.arange(-0.15, 0.11, 0.05))
            ...
            _, zini = ptravers_file.get_values(0,
                get_section_values=True, section_vars_indexes=section_vars)
            _, zfin = ptravers_file.get_values(-1,
                get_section_values=True, section_vars_indexes=section_vars)
            ...
            # init profile
            section_id = 0
            ax.plot(zini[reach_id][section_id][0], zini[reach_id][section_id][1],
                    label='Init', color='black', marker='*')
            # x = 1
            section_id = 4
            zmin = np.min(zini[reach_id][section_id][1])
            ax.plot(zfin[reach_id][section_id][0], zfin[reach_id][section_id][1]
                    - zmin, label='X = 1', color='red')
            # x = 5
            section_id = 20
            zmin = np.min(zini[reach_id][section_id][1])
            ax.plot(zfin[reach_id][section_id][0], zfin[reach_id][section_id][1]
                    - zmin, label='X = 5', color='green')
            # x = 10
            section_id = 40
            zmin = np.min(zini[reach_id][section_id][1])
            ax.plot(zfin[reach_id][section_id][0], zfin[reach_id][section_id][1]
                    - zmin, label='X = 10', color='blue')
            # gather all legends

...
class VnvStudy(AbstractVnvStudy):
    ...
    def _pre(self):
        ...
        # parallel
        cas = TelemacCas('t3d_border-ice.cas', get_dico('telemac3d'))
        cas.set('PARALLEL PROCESSORS', 4)
        self.add_study('vnv_par',
                       'telemac3d',
                       't3d_border-ice_par.cas',
                       cas=cas)
    ...
    def _check_results(self):
        ...
        self.check_epsilons('vnv_seq:T3DRES', 'f3d_border-ice.slf', eps=[1.E-6])
        self.check_epsilons('vnv_seq:ICERES', 'fce_border-ice.slf', eps=[1.E-6])

        # Comparison with the last time frame of the reference file.
        self.check_epsilons('vnv_par:T3DRES', 'f3d_border-ice.slf', eps=[0.1])
        self.check_epsilons('vnv_par:ICERES', 'fce_border-ice.slf', eps=[1.01])

        # Comparison between serial and parallel run.
        self.check_epsilons('vnv_seq:T3DRES', 'vnv_par:T3DRES', eps=[0.1])
        self.check_epsilons('vnv_seq:ICERES', 'vnv_par:ICERES', eps=[1.01])
    ...
    def _post(self):
        ...
        geo, _ = self.get_study_res('vnv_seq:T3DGEO')
        #res_ini = TelemacFile(self.get_study_file('vnv_ini:T2DRES'))
        res_seq = TelemacFile(self.get_study_file('vnv_seq:T3DRES'))

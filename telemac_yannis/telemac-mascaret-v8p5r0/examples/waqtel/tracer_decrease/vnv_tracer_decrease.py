...
#                   fig_name='img/res_ta1')
        ...
        # Computing theory values for degradation law over time for each point
        # of extraction
        theory = np.zeros(len(times))
        for itime, time in enumerate(times):
            theory[itime] = 200.*math.exp(-2.3/6.*time)

        #Initialising figure
        _, axe = plt.subplots(figsize=(10,5))

        # for each plot adding a history plot with a label node_(node_number)
        plot1d(axe, times, theory, marker='o',
               x_label='$t$ (h)',
               y_label='tracer concentration',
               plot_label='theory')

        # for each plot adding a history plot with a label node_(law)
        plot1d(axe, times, data2[0, :],
               x_label='$t$ (h)',
               y_label='tracer concentration',
               plot_label='$T_{90}$ in h')

        # for each plot adding a history plot with a label node_(law)
        plot1d(axe, times, data3[0, :],
               x_label='$t$ (h)',
               y_label='tracer concentration',
               plot_label='$k_1$ in h$^{-1}$')

        # for each plot adding a history plot with a label node_(law)
        plot1d(axe, times, data4[0, :],
               x_label='$t$ (h)',
               y_label='tracer concentration',
               plot_label='$k_1$ in day$^{-1}$')

        # for each plot adding a history plot with a label node_(law)
        plot1d(axe, times, data5[0, :],
               x_label='$t$ (h)',
               y_label='tracer concentration',

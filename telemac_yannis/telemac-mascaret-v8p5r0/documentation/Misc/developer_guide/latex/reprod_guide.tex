...
%(where all the parameters of the simulation are defined)
via the keyword \telkey{FINITE ELEMENT ASSEMBLY}.
...
Only \telemac{2D} can handle this choice at the moment.
%
\section{Methodology}
...
For that, we introduce the subroutine \telfile{GLOB\_VEC}
...
processing \telfile{PARCOM} or \telfile{PARCOM\_COMP}, corresponding to the
computation mode.
...
The part of the subroutine \telfile{BIEF\_DEF} in
...
subroutine \telfile{BIEF\_ALLVEC}.
%
...
the subroutine \telfile{OS}, which only verifies the structure
before calling the subroutine \telfile{OV}.
...
\telfile{OV\_COMP} is called, and the passed vectors
...
by the subroutine \telfile{OM} for matrix instead of \telfile{OS}
for other structures.
In \telfile{OM}, the storage and the type of the
element are verified to call the subroutine \telfile{OM1111}
...
to the subroutines \telfile{OV} or \telfile{OV\_COMP}.
In the compensated version, the only modification in
\telfile{OM} and \telfile{OM1111} is at the subroutine parameter
...
\begin{tikzpicture}[align=center]
	\node[draw] (n1) at (0,11) {\telfile{OS} \\ \tiny operations on structure};
	\node[draw] (n2) at (5,11) {\telfile{OM} \\ \tiny operations on matrix};
	\node[draw] (n3) at (5,9) {\telfile{OM1111}};
	\node[draw] (n5) at (0.5,6) {\telfile{OV} \\ \tiny operations on vectors};
	\node[draw] (n6) at (5.2,6) {\telfile{OV\_COMP}\\ \tiny operations on vectors and their errors};
...
addition are calculated by the subroutine \telfile{TWOSUM} which is added in \bief.
In practice, the computation of any vector
is realised in the subroutine \telfile{VECTOS}.
...
%
\begin{enumerate}
\item  \telfile{VECTOS} calls the subroutine \telfile{ASSVEC} that computes
...
In the original mode, only the vector \texttt{VEC} is passed into the \telfile{ASSVEC} call,
...
subroutine \telfile{TWOSUM} (lines~\ref{ln:ass_2sum1} and~\ref{ln:ass_2sum2})
...
is the interface node assembly that is launched by the subroutine \telfile{PARCOM}.
This later calls \telfile{PARCOM2}, which then calls \telfile{PARACO} twice
...
\telfile{PARCOM2\_COMP} and \telfile{PARACO\_COMP} replace
\telfile{PARCOM2} and \telfile{PARACO}, respectively.\\
%
The modifications of \telfile{PARCOM\_COMP} are the addition of the
error component parameter and  the suppression of the second call of
\telfile{PARACO} which is no more needed because our corrections
recover the solution of continuity between the subdomains.
\\
%
The main modifications occur in \telfile{PARACO\_COMP} and are presented in Listing~\ref{lst:ass_IP}.
The Fortran subroutines \telfile{P\_IREAD}, \telfile{P\_IWRIT} and \telfile{P\_WAIT\_PARACO}
call respectively the MPI operations:
\telfile{MPI\_IRECV}, \telfile{MPI\_ISEND} and \telfile{MPI\_WAITALL}.\\
...
are declared in \telfile{BIEF\_DEF} to receive and send the
...
Here the assembly is realised with the \telfile{TWOSUM} subroutine
...
\begin{lstlisting}[language=TelFortran,
caption={The IP assembly in \telfile{PARACO\_COMP}},
label={lst:ass_IP},escapechar=\$]
! Receive step
DO IL=1,NB_NEIGHB
  IKA = NB_NEIGHB_PT(IL)
  IPA = LIST_SEND(IL)
...
$\Hilight$       CALL TWOSUM(V1(II,J),BUF_RECV(K,IL)
$\Hilight$   &       ,V1(II,J),ERROR1) $\label{ln:paraco_err1}$
$\Hilight$       CALL TWOSUM(ERRV(II),BUF_RECV_ERR(K,IL)
...
%
\item \textbf{Note 4.} For the studied \textit{gouttedo} test case, the other
calls of \telfile{PARCOM} are
in the subroutines \telfile{PROPAG, MASBAS2D, MATRBL}. In the compensation mode,
...
provided by the subroutine \telfile{GRACJG}.
The modifications impact the computations of the dot product
in function \telfile{P\_DOTS}, and the EBE
matrix-vector product in subroutine \telfile{MATRBL},
which are called by \telfile{GRACJG}.\\
%
\textbf{i) The dot product $X \cdot Y$}\\
Subroutine \telfile{P\_DOTS} calls the corresponding dot product according to the computation mode,
...
the dot product is computed with the function \telfile{DOT} as:
...
is computed partially by each subdomain, in function \telfile{P\_DOT}, as:
...
the global dot product by the MPI dynamic reduction in \telfile{P\_DSUM}.\\
%
In the compensated mode, a twice more accurate scalar product is
computed.
In sequential, function \telfile{DOT\_COMP} computes such
...
errors, to return a pair [data, error] in subroutine \telfile{P\_DOTPAIR}.
%
\begin{lstlisting}[language=TelFortran,
...
%
The $M \times V$ product is launched by subroutine \telfile{MATRBL}
called in the conjugate gradient.
Three subroutines are then called: \telfile{MATVEC}, \telfile{MATVCT}
and \telfile{MV0303}.\\
...
In \telfile{MV0303}, the Hadamard product $DA \times Y$ computed
by \telfile{OV\_COMP} is modified to update the associated errors.
%
...
in subroutine \telfile{ASSVEC} (Listing~\ref{lst:ass_FE}).
%
The final step assembles the matrix-vector product
at the interface point in \telfile{MATRBL} with a \telfile{PARCOM} call
...
the \bief library, the parallel library and \telemac{2D} module
...
\telfile{ALMESH}, \telfile{ASSVEC}, \telfile{BIEF}, \telfile{BIEF\_ALLVEC},
\telfile{BIEF\_DEF}, \telfile{MATRBL}, \telfile{MATRIX},
\telfile{MATVEC}, \telfile{OM}, \telfile{MV\_0303}, \telfile{OM\_1111}, \telfile{OS}, \telfile{P\_DOTS}, \telfile{PARINI}, \telfile{PRECD1},
\telfile{SOLVE}, \telfile{VECTOS}.
\item \textbf{\textit{Added:}}
\telfile{OV\_COMP}, \telfile{DOT\_COMP}, \telfile{P\_DOT\_COMP}, \telfile{PARCOM\_COMP}, \telfile{PARCOM2\_COMP},
\telfile{PARACO\_COMP},\telfile{TWOSUM}, \telfile{TWOPROD}.
\end{itemize}
%
\item \textbf{Parallel library.}
\begin{itemize}
\item \textbf{\textit{Modified:}} \telfile{INTERFACE\_PARALLEL}.
\item \textbf{\textit{Added:}} \telfile{P\_DSUM\_ERR}.
\end{itemize}
%
\item \textbf{\telemac{2D} module.}
\begin{itemize}
\item \textbf{\textit{Modified:}}
\telfile{LECDON\_TELEMAC}, \telfile{MASBAS2D}, \telfile{PROPAG}, \texttt{telemac2d.dico}.
\end{itemize}
%
\end{description}
